...
namespace TNS.AdExpress.Web.Helper.DataAccess
{
    ...
    public class VersionDataAccess
    {
        ...
        public static DataSet GetVersions(ICollection versions, WebSession webSession, DBCst.Vehicles.names vehicle, MediaSchedulePeriod period)
        {
            ...
            DateTime begin;
            if (period != null)
                begin = Core.Utilities.Dates.GetPeriodBeginningDate(period.Begin.ToString("yyyyMMdd"), webSession.PeriodType);
            else
                begin = Core.Utilities.Dates.GetPeriodBeginningDate(webSession.PeriodBeginningDate, webSession.PeriodType);
        ...
        {
            DateTime today = DateTime.Now.Date;
            DateTime begin = Core.Utilities.Dates.GetPeriodBeginningDate(webSession.PeriodBeginningDate, webSession.PeriodType);
        ...
        private static DataSet GetPressVersionsDetails(ICollection versions, WebSession webSession, string dbTbl, string dbSchema, string tablePrefixe)
        {
            ...
            string dateBegin = Core.Utilities.Dates.GetPeriodBeginningDate(webSession.PeriodBeginningDate, webSession.PeriodType).ToString("yyyyMMdd");
            string dateEnd = Core.Utilities.Dates.GetPeriodEndDate(webSession.PeriodEndDate, webSession.PeriodType).ToString("yyyyMMdd");
            #endregion
            ...
            #region ancienne version
                ...
                + ")  "
                //Product Right
                + Core.Utilities.SQLGenerator.getAnalyseCustomerProductRight(webSession, tablePrefixe, true)
                //media rights
                + "  " + Core.Utilities.SQLGenerator.getAnalyseCustomerMediaRight(webSession, tablePrefixe, true)
                + "group by  ID_SLOGAN";
        ...
        private static DataSet GetAPPMVersions(ICollection versions, WebSession webSession, string dbTbl, string dbSchema, string tablePrefixe)
        {
            ...
            query = "SELECT id_slogan as id, visual as visual,date_media_num as dateKiosque,date_cover_num as dateCover, id_media as idmedia"
                ...
                // Products to exclude
                + " " + Core.Utilities.SQLGenerator.GetAdExpressProductUniverseCondition(AdExpressUniverse.EXCLUDE_PRODUCT_LIST_ID, tablePrefixe, true, false)
