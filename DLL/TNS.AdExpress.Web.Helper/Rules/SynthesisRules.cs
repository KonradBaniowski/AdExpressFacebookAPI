...
namespace TNS.AdExpress.Web.Helper.Rules
{
    /// <summary>
    ...
    public class SynthesisRules
    {
        ...
        public static Hashtable GetData(WebSession webSession, IDataSource dataSource, int dateBegin, int dateEnd, Int64 baseTarget, Int64 additionalTarget, Int64 idProduct, bool mediaAgencyAccess)
        {
            ...
            try
            {
                ...
                synthesisTable = SynthesisDataAccess.GetData(webSession, dataSource, dateBegin, dateEnd, baseTarget, additionalTarget, idProduct, mediaAgencyAccess).Tables[0];
                #endregion
                ...
                if (synthesisTable != null && synthesisTable.Rows.Count > 0)
                {
                    ...
                    int dateBeginning = int.Parse(Core.Utilities.Dates.GetPeriodBeginningDate(webSession.PeriodBeginningDate, webSession.PeriodType).ToString("yyyyMMdd"));
                    int dateEnding = int.Parse(Core.Utilities.Dates.GetPeriodEndDate(webSession.PeriodEndDate, webSession.PeriodType).ToString("yyyyMMdd"));
                    //Formatting the date in the format e.g 12/12/2005
                    if (dateBeginning.ToString().Length > 0)
                        startDate = Core.Utilities.Dates.DateToString(new DateTime(int.Parse(dateBeginning.ToString().Substring(0, 4)), int.Parse(dateBeginning.ToString().Substring(4, 2)), int.Parse(dateBeginning.ToString().Substring(6, 2))), webSession.SiteLanguage);
                    if (dateEnding.ToString().Length > 0)
                        endDate = Core.Utilities.Dates.DateToString(new DateTime(int.Parse(dateEnding.ToString().Substring(0, 4)), int.Parse(dateEnding.ToString().Substring(4, 2)), int.Parse(dateEnding.ToString().Substring(6, 2))), webSession.SiteLanguage);
                    #endregion
                    ...
                    DataTable groupsTable = DataAccess.Functions.SelectDistinct("groups", synthesisTable, groupArray);
                    ...
                    DataTable mediaTable = DataAccess.Functions.SelectDistinct("media", synthesisTable, "id_media");
                    if (mediaTable != null)
                    ...
                    synthesisPDV = SynthesisDataAccess.UniversGroupInvestment(webSession, dataSource, dateBegin, dateEnd, baseTarget, additionalTarget, groupIds.Remove(groupIds.Length - 1, 1)).Tables[0];
            ...
            {
                throw (new SynthesisRulesException("Error while formatting the data for APPM Synthesis ", err));
            }
            ...
            {//Droits versions
                try
                {
                    ...
                    if (idVersion != null && idVersion.Length > 0)
                        synthesisTable = SynthesisDataAccess.GetData(webSession, dataSource, dateBeginning, dateEnding, baseTarget, additionalTarget, mediaAgencyAccess, idVersion).Tables[0];
                    #endregion
                    ...
                    if (synthesisTable != null && synthesisTable.Rows.Count > 0)
                    {
                        ...
                        {
                            investProduct = SynthesisDataAccess.GetProductInvestment(webSession, dataSource, dateBegin, dateEnd, baseTarget, synthesisTable.Rows[0]["id_product"].ToString()).Tables[0];
                ...
                {
                    throw (new SynthesisRulesException("Error while formatting the data for APPM Synthesis by Version ", err));
                }
            ...
            if (webSession.CustomerLogin.CustormerFlagAccess(TNS.AdExpress.Constantes.DB.Flags.ID_SLOGAN_ACCESS_FLAG))
            {
                ...
                try
                {
                    ...
                    if (versions != null && versions.Count > 0)
                        synthesisTable = SynthesisDataAccess.GetData(webSession, dataSource, dateBeginning, dateEnding, baseTarget, additionalTarget, mediaAgencyAccess, versions).Tables[0];
                    #endregion
                    ...
                    foreach (Object version in versions)
                    {
                        ...
                        if (synthesisTableTmp != null && synthesisTableTmp.Rows.Count > 0)
                        {
                            ...
                            //to select distinct group ids and groups using the SelectDistinct function
                            string[] groupArray = { "id_group_", "group_" };
                            DataTable groupsTable = DataAccess.Functions.SelectDistinct("groups", synthesisTableTmp, groupArray);
                            foreach (DataRow dr in groupsTable.Rows)
                            {
                                groupIds += dr["id_group_"].ToString() + ",";
                                groups += dr["group_"].ToString() + ",";
                            ...
                            #region getting number of media
                            DataTable mediaTable = DataAccess.Functions.SelectDistinct("media", synthesisTableTmp, "id_media");
                            if (mediaTable != null)
                                numberOfMedias = mediaTable.Rows.Count;
                            #endregion
                            ...
                            {
                                investProduct = SynthesisDataAccess.GetProductInvestment(webSession, dataSource, dateBegin, dateEnd, baseTarget, synthesisTableTmp.Rows[0]["id_product"].ToString()).Tables[0];
                                if (investProduct != null && investProduct.Rows.Count > 0 && !investProduct.Rows[0][UnitsInformation.List[WebCste.CustomerSessions.Unit.euro].Id.ToString()].ToString().Equals(""))
                                {
                                    tempInvestProduct = Convert.ToInt64(investProduct.Rows[0][UnitsInformation.List[WebCste.CustomerSessions.Unit.euro].Id.ToString()]);
                                    if (tempInvestProduct > 0)
