    ...
    public class CustomerWebException : System.Exception{
        ...
        public void SendMail()
        {
            ...
            else
            {
                ...
                if (webSession.isAdvertisersSelected())
                {
                    //Selection agence media principale
                    if (webSession.PrincipalAdvertisingAgnecyUniverses.Count == 1)
                    {
                        body += "<u>" + GestionWeb.GetWebWord(2817, webSession.SiteLanguage) + "</u><br>";
                        body += TNS.AdExpress.Web.Functions.DisplayUniverse.ToHtml(webSession.PrincipalAdvertisingAgnecyUniverses[0], webSession.SiteLanguage, webSession.DataLanguage, webSession.Source, 600);
                    }
                }
