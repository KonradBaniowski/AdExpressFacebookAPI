			...
			if(webSession==null){
				...
				if (webSession.isAdvertisersSelected()) {
					...
					if (webSession.PrincipalProductUniverses.Count == 1) {
						...
						body += TNS.AdExpress.Web.Functions.DisplayUniverse.ToHtml(webSession.PrincipalProductUniverses[0], webSession.SiteLanguage, new OracleConnection(webSession.CustomerLogin.OracleConnectionString), 600);
					}
				...
				if (webSession.isReferenceAdvertisersSelected()) {
					...
					if (webSession.SecondaryProductUniverses.Count == 1) {
						...
						body += "<u>" + GestionWeb.GetWebWord(universeCodeTitle, webSession.SiteLanguage) + "</u><br>";
						body += TNS.AdExpress.Web.Functions.DisplayUniverse.ToHtml(webSession.SecondaryProductUniverses[0], webSession.SiteLanguage, new OracleConnection(webSession.CustomerLogin.OracleConnectionString), 600);
					}
						...
						for (int k = 0; k < webSession.SecondaryProductUniverses.Count; k++) {
							...
							body += "<u>" + GestionWeb.GetWebWord(universeCodeTitle, webSession.SiteLanguage) + "</u><br>";
							body += TNS.AdExpress.Web.Functions.DisplayUniverse.ToHtml(webSession.SecondaryProductUniverses[k], webSession.SiteLanguage, new OracleConnection(webSession.CustomerLogin.OracleConnectionString), 600);
						}
					}
				}
				if (webSession.CurrentModule == TNS.AdExpress.Constantes.Web.Module.Name.INDICATEUR) {
