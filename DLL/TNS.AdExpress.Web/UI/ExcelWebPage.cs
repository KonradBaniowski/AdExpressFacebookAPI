...
namespace TNS.AdExpress.Web.UI{
	...
	public class ExcelWebPage: ResultWebPage{
		...
		#region GetExcelHeader (Ancienne version)
		/// <summary>
		...
		private static string GetExcelHeader_ORIGINAL(WebSession webSession, bool levelDetail, bool units, bool resultType, bool insert, bool media, bool dateFormatText, string periodBeginning, string periodEnd, string title){
			...
			try{
				...
				if(!(title.Length==0)){
					t.Append("<tr>");
					t.Append("<td class=\"excelDataTitle\">"+title+"</td>");
					t.Append("</tr>");
				}
				t.Append("<tr>");
				t.Append("<td class=\"excelDataItalic\">"+GestionWeb.GetWebWord(464,webSession.SiteLanguage)+"</td>");
				t.Append("</tr>");
				
				#region Bilan de campagne (APPM)
				if(webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.BILAN_CAMPAGNE) {
					// Vague selectionée pour APPL
					if (webSession.IsWaveSelected()){
						string waveText= ((LevelInformation)webSession.SelectionUniversAEPMWave.FirstNode.Tag).Text;							
						if(waveText.Length>0){
							t.Append("<tr>");
							t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1762,webSession.SiteLanguage)+"  "+ waveText +"</td>");
							t.Append("</tr>");
						}
					}
					// Cible selectionée pour APPM
					if(!(webSession.CurrentTab==APPM.affinities)){									
						if (webSession.IsTargetSelected()){
							string targetText= ((LevelInformation)webSession.SelectionUniversAEPMTarget.LastNode.Tag).Text;								
							if(targetText.Length>0){
								t.Append("<tr>");
								t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1672,webSession.SiteLanguage)+" : "+ targetText +"</td>");
								t.Append("</tr>");
							}
						}
					}
				...
				#region Famille
				if(webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_TELEVISION 
					||webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_RADIO
					||webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_PRESSE
					||webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_PAN_EURO) {
					try	{
						string family= ((LevelInformation)webSession.SelectionUniversProduct.FirstNode.Tag).Text;							
						if(family.Length>0){
							t.Append("<tr>");
							t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1103,webSession.SiteLanguage)+" : "+ family +"</td>");
							t.Append("</tr>");
						}
					}
				...
				#region Jour nommé, tranche horaires, format (Radio ou TV)
				if(webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_TELEVISION 
					|| webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_PAN_EURO
					|| webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_RADIO) {
					...
					else if(webSession.NamedDay==CstWeb.Repartition.namedDay.Total)
					...
					if(namedDay.Length>0){
						t.Append("<tr>");
						t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1574,webSession.SiteLanguage)+" : "+ namedDay +"</td>");
						t.Append("</tr>");
					}
					#endregion
					...
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1575,webSession.SiteLanguage)+" : "+ TNS.AdExpress.Web.Functions.Dates.GetTimeSlice(webSession,webSession.TimeInterval) +"</td>");
					t.Append("</tr>");
					#endregion

					#region Format
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1420,webSession.SiteLanguage)+" : "+ TNS.AdExpress.Web.Functions.Dates.GetFormat(webSession,webSession.Format) +"</td>");
					t.Append("</tr>");
					#endregion
				...
				// Supports pour Portefeuille d'un support
				if(webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.ALERTE_PORTEFEUILLE||webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.ANALYSE_PORTEFEUILLE){					
					OracleConnection connection=new OracleConnection(webSession.CustomerLogin.OracleConnectionString);
					TNS.AdExpress.Classification.DataAccess.MediaBranch.PartialMediaListDataAccess supportSelected=new TNS.AdExpress.Classification.DataAccess.MediaBranch.PartialMediaListDataAccess((((LevelInformation)webSession.ReferenceUniversMedia.FirstNode.Tag).ID).ToString(),webSession.SiteLanguage,connection);
					string support=supportSelected[((LevelInformation)webSession.ReferenceUniversMedia.FirstNode.Tag).ID];
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(18,webSession.SiteLanguage)+" : "+ support+"</td>");
					t.Append("</tr>");
				}
				...
				if(media){			
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(190,webSession.SiteLanguage)+" : "+ StringMediaSelected(webSession) +"</td>");
					t.Append("</tr>");
				}
				
				// Date format to be like forexample novembre 2004 - janvier 2005
				string startDate="";
				string endDate="";
				if(dateFormatText){
					...
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1541,webSession.SiteLanguage)+" : "+startDate);
					if(!startDate.Equals(endDate))t.Append(" - "+endDate);
					t.Append("</td>");
					t.Append("</tr>");
				}
				else{
				...
				if(resultType){
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1493,webSession.SiteLanguage)+" : "+StringTypeResult(webSession)+"</td>");
					t.Append("</tr>");
				}
				
				//Insertion pour presse
				if(insert){
					if(vehicleType==ClassificationCst.DB.Vehicles.names.press||vehicleType==ClassificationCst.DB.Vehicles.names.internationalPress){
						...
						t.Append("<tr>");
						t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1400,webSession.SiteLanguage)+" "+GestionWeb.GetWebWord(code,webSession.SiteLanguage)+"</td>");
						t.Append("</tr>");
					}
				}
				
				// Unité
				if(webSession.PreformatedTable==WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.units_X_Format||
					webSession.PreformatedTable==WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.units_X_NamedDay ||
					webSession.PreformatedTable==WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.units_X_TimeSlice)
					units=false;
				if(units){
					t.Append("<tr>");
					t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1313,webSession.SiteLanguage)+" "+GestionWeb.GetWebWord((int)TNS.AdExpress.Constantes.Web.CustomerSessions.XLSUnitsTraductionCodes[webSession.Unit],webSession.SiteLanguage)+"</td>");
					t.Append("</tr>");
				}
				
				// Detail de produit, média, unité
				if(levelDetail){				
					t.Append("<tr>");
					if(webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_PRESSE||
						webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_RADIO||
						webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_TELEVISION||
						webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DE_BORD_PAN_EURO ||
						webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DYNAMIQUE) {
						
						switch(webSession.PreformatedTable){
								// cases where media detail is shown
							...
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.media_X_Year:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.mediaYear_X_Cumul:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.mediaYear_X_Mensual:
								t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1150,webSession.SiteLanguage)+" "+StringLevelMedia(webSession)+"</td>");
								break;
								//cases where unit detail is shown 
							...
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.units_X_TimeSlice :
								//string Vehicle = ((LevelInformation)webSession.SelectionUniversMedia.FirstNode.Tag).ID.ToString();
								//ClassificationCst.DB.Vehicles.names vehicleType = (ClassificationCst.DB.Vehicles.names)int.Parse(Vehicle);
								//for press we have different units as compared to radio and tv
								if(vehicleType!=ClassificationCst.DB.Vehicles.names.press) {
									t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1313,webSession.SiteLanguage)+"&nbsp;"+GestionWeb.GetWebWord(1423,webSession.SiteLanguage)+",&nbsp;"+GestionWeb.GetWebWord(861,webSession.SiteLanguage)+",&nbsp; "+GestionWeb.GetWebWord(869,webSession.SiteLanguage)+"</td>");
								}
								break;
								//cases where product detail is shown
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.product_X_Year:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.productYear_X_Cumul:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.productYear_X_Media:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.productYear_X_Mensual:
							...
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.sector_X_TimeSlice :
								t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1124,webSession.SiteLanguage)+" "+WebFunctions.ProductDetailLevel.LevelProductToExcelString(webSession)+"</td>");
								break;
								//cases where both product and media detail are shown
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.productMedia_X_Year:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.productMedia_X_YearMensual:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.mediaProduct_X_Year:
							case WebConstantes.CustomerSessions.PreformatedDetails.PreformatedTables.mediaProduct_X_YearMensual:
								t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1124,webSession.SiteLanguage)+" "+WebFunctions.ProductDetailLevel.LevelProductToExcelString(webSession)+"</td>");
								t.Append("</tr>");
								t.Append("<tr>");
								t.Append("<td class=\"excelData\">"+GestionWeb.GetWebWord(1150,webSession.SiteLanguage)+" "+StringLevelMedia(webSession)+"</td>");
            ...
            if (periodDisplayLevel != -1) {
                switch ((WebConstantes.CustomerSessions.Period.DisplayLevel)periodDisplayLevel) {
                    case CstWeb.CustomerSessions.Period.DisplayLevel.weekly:
                        ...
                        html.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>" + GestionWeb.GetWebWord(2291, webSession.SiteLanguage) + " </font> " + begin.Date.ToString("dd/MM/yyyy"));
                        if (!begin.Equals(end))
		...
		public static string GetProductSelected(WebSession webSession){
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(814,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(813,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(1585,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(815,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(965,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(966,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(964,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			//    // Affichage du TreeNode
			//    t.Append(TNS.AdExpress.Web.Functions.DisplayTreeNode.ToExcel(webSession.CurrentUniversAdvertiser));
			//}
			#endregion
			...
			//    }
			//    // Affichage du TreeNode
			//    t.Append(TNS.AdExpress.Web.Functions.DisplayTreeNode.ToExcel(webSession.SelectionUniversProduct));
			...
			//    }
			//    // Affichage du TreeNode
			//    t.Append(TNS.AdExpress.Web.Functions.DisplayTreeNode.ToExcel(webSession.CurrentUniversProduct));
			//}
			#endregion
			...
			//        t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(1195,webSession.SiteLanguage)+"</font></td></tr>");
			//    }
			//    // Affichage du TreeNode
			//    t.Append(TNS.AdExpress.Web.Functions.DisplayTreeNode.ToExcel(webSession.ReferenceUniversAdvertiser));
			//}
			#endregion
			...
			//            }
			//            if(webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.INDICATEUR || webSession.CurrentModule==TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DYNAMIQUE ){
			//                t.Append("<tr><td colspan=4 class=\"excelData\"><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(1196,webSession.SiteLanguage)+"</font></td></tr>");
			//            }
			//            if(webSession.CurrentModule!=TNS.AdExpress.Constantes.Web.Module.Name.INDICATEUR && webSession.CurrentModule!=TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DYNAMIQUE ){
			//                t.Append("<TR>");
			...
			//            // Affichage du TreeNode	
			//            if(webSession.CurrentModule!=TNS.AdExpress.Constantes.Web.Module.Name.INDICATEUR && webSession.CurrentModule!=TNS.AdExpress.Constantes.Web.Module.Name.TABLEAU_DYNAMIQUE ){
			//                t.Append(TNS.AdExpress.Web.Functions.DisplayTreeNode.ToExcel((TreeNode)(((TNS.AdExpress.Web.Core.Sessions.CompetitorAdvertiser)webSession.CompetitorUniversAdvertiser[idAdvertiser]).TreeCompetitorAdvertiser)));
			//            }
			//            else{
			//                t.Append(TNS.AdExpress.Web.Functions.DisplayTreeNode.ToExcel(tree));
			//            }
			//            idAdvertiser++;
			//        }
			//        else{
		...
		private static string GetProductSelectedForCreationsPopUp(WebSession webSession, Int64 idElement,int level){
			...
			if(webTexId > 0){
				...
				if(elementIsAdvertiser){ // Annonceur sélectionné
					AdExClassification.ProductBranch.PartialAdvertiserLevelListDataAccess elementName = new AdExClassification.ProductBranch.PartialAdvertiserLevelListDataAccess(idElement.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
					t.Append("<TR><TD colspan=4 class=\"excelData\" >"+ elementName[idElement].ToString() +"</TD></TR>");
				}
				else{ // Produit sélectionné
					AdExClassification.ProductBranch.PartialProductLevelListDataAccess elementName = new AdExClassification.ProductBranch.PartialProductLevelListDataAccess(idElement.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
					t.Append("<TR><TD colspan=4 class=\"excelData\" >"+ elementName[idElement].ToString() +"</TD></TR>");
				}
			}
			return(t.ToString());
				...
				if(long.Parse(de.Value.ToString())>-1){
					...
					switch(mediaSelectLabel){
						case DBCst.Fields.ID_VEHICLE :
							elementName = new AdExClassification.MediaBranch.PartialVehicleListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case DBCst.Fields.ID_CATEGORY :
							elementName = new AdExClassification.MediaBranch.PartialCategoryListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case DBCst.Fields.ID_INTEREST_CENTER :
							elementName = new AdExClassification.MediaBranch.PartialInterestCenterListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case DBCst.Fields.ID_MEDIA_SELLER :
							elementName = new AdExClassification.MediaBranch.PartialMediaSellerListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case DBCst.Fields.ID_MEDIA :
							elementName = new AdExClassification.MediaBranch.PartialMediaListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case  DBCst.Fields.ID_ADVERTISER :
							elementName = new AdExClassification.ProductBranch.PartialAdvertiserLevelListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case  DBCst.Fields.ID_BRAND :
							elementName = new AdExClassification.ProductBranch.PartialBrandLevelListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case  DBCst.Fields.ID_PRODUCT :
							elementName = new AdExClassification.ProductBranch.PartialProductLevelListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case  DBCst.Fields.ID_SECTOR :
							elementName = new AdExClassification.ProductBranch.PartialSectorLevelListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case  DBCst.Fields.ID_SUBSECTOR :
							elementName = new AdExClassification.ProductBranch.PartialSubSectorLevelListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case  DBCst.Fields.ID_GROUP_ :
							elementName = new AdExClassification.ProductBranch.PartialGroupLevelListDataAccess(mediaSelectId.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
				...
				if(webSession.AdNetTrackSelection.Id.ToString().Length>0){
					switch(webSession.AdNetTrackSelection.SelectionType){
						case AdNetTrackMediaSchedule.Type.advertiser:
							AdExClassification.ProductBranch.PartialAdvertiserLevelListDataAccess advertiser = new AdExClassification.ProductBranch.PartialAdvertiserLevelListDataAccess(webSession.AdNetTrackSelection.Id.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
						...
						case AdNetTrackMediaSchedule.Type.product:
							AdExClassification.ProductBranch.PartialProductLevelListDataAccess product = new AdExClassification.ProductBranch.PartialProductLevelListDataAccess(webSession.AdNetTrackSelection.Id.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
		...
		public static string GetExcelHeaderForCreationsPopUpFromMediaPlan(WebSession webSession, bool dateFormatText, string periodBeginning, string periodEnd, ListDictionary mediaImpactedList, int idVehicle){
			...
			try{
				...
				t.Append(GetDateSelected(webSession, currentModule, dateFormatText, periodBeginning, periodEnd));			
				// Media
				AdExClassification.MediaBranch.PartialVehicleListDataAccess vehicleName = new AdExClassification.MediaBranch.PartialVehicleListDataAccess(idVehicle.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
		...
		public static string GetExcelHeaderForCreationsPopUpFromPortofolio(WebSession webSession, bool dateFormatText, string periodBeginning, string periodEnd, int idVehicle, int idMedia, bool allPeriod, string nameDay, string codeEcran){
			...
			try{
				...
				t.Append("<TR><TD colspan=4 class=\"excelData\" ><font class=txtBoldGrisExcel>"+ GestionWeb.GetWebWord(190,webSession.SiteLanguage) +" : </font> "+ vehicleName[idVehicle].ToString() +"</TD></TR>");
				// Support
				AdExClassification.MediaBranch.PartialMediaListDataAccess mediaName = new AdExClassification.MediaBranch.PartialMediaListDataAccess(idMedia.ToString(),webSession.SiteLanguage,(OracleConnection)webSession.Source.GetSource());
