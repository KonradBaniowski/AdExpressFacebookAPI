...
namespace TNS.AdExpress.Web.UI.Results {
    ...
    public class GenericMediaScheduleUI
    {
        ...
        public static MediaPlanResultData GetHtml(object[,] tab,WebSession webSession, MediaSchedulePeriod period, string zoom) {
            ...
            Int64 i = -1;
            return (GetHtml(tab, webSession, period, zoom, null, ref i, false, -1));
        ...
        {
            return GetHtml(tab, webSession, period, string.Empty, header,ref nbWeek, true, idVehicle);
        }
        ...
        /// <returns>Media plan result (html code + versions codes if required)</returns>
        private static MediaPlanResultData GetHtml(object[,] tab,WebSession webSession, MediaSchedulePeriod period, string zoom, StringBuilder header, ref Int64 nbWeek, bool isExport, int idVehicle) {
                        ...
                        switch(j) {
                            ...
                            case DetailledMediaPlan.L1_COLUMN_INDEX:
                                if(tab[i,j]!=null) {
                                    ...
                                    if(i==DetailledMediaPlan.TOTAL_LINE_INDEX) classe="L0";
                                        ...
                                        classe="L1";
                                        if(!isExport && tab[i,FrameWorkResultConstantes.DetailledMediaPlan.L1_ID_COLUMN_INDEX]!=null) {
                            ...
                            case DetailledMediaPlan.L2_COLUMN_INDEX:
                                if(tab[i,j]!=null) {
                                    ...
                                    if (!isExport && tab[i, FrameWorkResultConstantes.DetailledMediaPlan.L2_ID_COLUMN_INDEX] != null) {
                                    ...
                                    // On traite le cas des pages
                                    totalUnit=WebFunctions.Units.ConvertUnitValueToString(tab[i,FrameWorkResultConstantes.DetailledMediaPlan.TOTAL_COLUMN_INDEX].ToString(),webSession.Unit);
                                    t.Append("\r\n\t<tr>\r\n\t\t<td class=\"L2\">&nbsp;"+tab[i,FrameWorkResultConstantes.DetailledMediaPlan.L2_COLUMN_INDEX]+"</td><td class=\"L2nb\">"+totalUnit+"</td><td class=\"L2nb\">"+((double)tab[i,FrameWorkResultConstantes.DetailledMediaPlan.PDM_COLUMN_INDEX]).ToString("0.00")+"</td>"+((!isExport)?((showVersions)?"<td align=\"center\" class=\"L2\">"+tmpHtml+"</td>":"")+"<td align=\"center\" class=\"L2\">"+tmpHtml2+"</td>":string.Empty));
                                    currentCategoryName=tab[i,FrameWorkResultConstantes.DetailledMediaPlan.L2_COLUMN_INDEX].ToString();
                                    //aller aux colonnes du calendrier d'action
                                    //MAJ GR : totaux par années si nécessaire
                                    for(k=1;k<=nbColYear;k++) {
                            ...
                            case DetailledMediaPlan.L3_COLUMN_INDEX:
                                if(tab[i,j]!=null) {
                                    ...
                                    if (!isExport && tab[i, FrameWorkResultConstantes.DetailledMediaPlan.L3_ID_COLUMN_INDEX] != null) {
