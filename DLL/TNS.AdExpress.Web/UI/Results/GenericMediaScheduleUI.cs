...
namespace TNS.AdExpress.Web.UI.Results {
    ...
    public class GenericMediaScheduleUI
    {
        ...
        public static MediaPlanResultData GetAdNetTrackHtml(WebSession webSession, MediaSchedulePeriod period, object[,] tab, string zoomDate)//, Page page, string url, string urlParameters, Int64 moduleId, int universId, string currentPage)
        {
            ...
            int nbtot = WebFunctions.Units.ConvertUnitValueToString(tab[1, FrameWorkResultConstantes.DetailledMediaPlan.TOTAL_COLUMN_INDEX], webSession.Unit, fp).Length;
            ...
            for (j = FIRST_PERIOD_INDEX; j < nbColTab; j++)
            {
                ...
                dateHtml.AppendFormat("<td class=\"{0}\">{1}</td>"
                    , dayClass
                    , DayString.GetCharacters(dt, cultureInfo, 1));
            }
            dateHtml.Append("\r\n\t</tr>");
        ...
        public static MediaPlanResultData GetExcel(object[,] tab, WebSession webSession, MediaSchedulePeriod period, string zoom, bool showValue, int periodDisplayLevel, bool isCreative)
        {
            ...
            string periodClass;
            CultureInfo cultureInfo = new CultureInfo(WebApplicationParameters.AllowedLanguages[webSession.SiteLanguage].Localization);
            IFormatProvider fp = WebApplicationParameters.AllowedLanguages[webSession.SiteLanguage].CultureInfoExcel;
            #endregion
            ...
            i = 0;
            UnitInformation unit = UnitsInformation.Get(webSession.Unit);
            try
            {
                        ...
                        switch (j)
                        {
                            ...
                            case DetailledMediaPlan.L1_COLUMN_INDEX:
                                if (tab[i, j] != null)
                                {
                                    ...
                                    if (webSession.Unit != CustomerSessions.Unit.duration)
                                    {
                                        totalUnit = WebFunctions.Units.ConvertUnitValueToString(tab[i, FrameWorkResultConstantes.DetailledMediaPlan.TOTAL_COLUMN_INDEX], webSession.Unit, fp);
                                    }
                                    else
                                    {
                                        totalUnit = string.Format(fp, unit.StringFormat, Convert.ToDouble(tab[i, FrameWorkResultConstantes.DetailledMediaPlan.TOTAL_COLUMN_INDEX]));
                                    }
                                    ...
                                    {
                                        if (unit.Id != CustomerSessions.Unit.duration)
                                        {
                                            t.Append("<td class=\"" + classe + "nb\" nowrap>" + WebFunctions.Units.ConvertUnitValueToString(tab[i, j + 10 + k], webSession.Unit, fp) + "</td>");
                                        }
                                        else
                                        {
                                            t.Append("<td class=\"" + classe + "nb\" nowrap>" + string.Format(fp, unit.StringFormat, Convert.ToDouble(tab[i, j + 10 + k])) + "</td>");                                            
                                        }
                                    }
                                    j = j + 10 + nbColYear;
                                    //fin MAJ
                            ...
                            case DetailledMediaPlan.L2_COLUMN_INDEX:
                                if (tab[i, j] != null)
                                {
                                    ...
                                    for (k = 1; k <= nbColYear; k++)
                                    {
                                        ...
                                        {
                                            t.Append("<td class=\"L2Xnb\">" + string.Format(fp, unit.StringFormat, Convert.ToDouble(tab[i, j + 9 + k])) + "</td>");                                                                                    
                                        }
                                    }
                                    j = j + 9 + nbColYear;
                                    //fin MAJ
                            ...
                            case DetailledMediaPlan.L3_COLUMN_INDEX:
                                if (tab[i, j] != null)
                                {
                                    ...
                                    for (k = 1; k <= nbColYear; k++)
                                    {
                                        t.Append("<td class=\"L3Xnb\">" + string.Format(fp, unit.StringFormat, Convert.ToDouble(tab[i, j + 8 + k])) + "</td>");
                                    }
                                    j = j + 8 + nbColYear;
                                    //fin MAJ
                                }
                            ...
                            case DetailledMediaPlan.L4_COLUMN_INDEX:
                                ...
                                for (k = 1; k <= nbColYear; k++)
                                {
                                    t.Append("<td class=\"L4Xnb\">" + string.Format(fp, unit.StringFormat, Convert.ToDouble(tab[i, j + 7 + k])) + "</td>");
                                }
                                //fin MAJ
                            ...
                            default:
                                ...
                                if (tab[i, j].GetType() == typeof(MediaPlanItem))
                                {
                                    switch (((MediaPlanItem)tab[i, j]).GraphicItemType)
                                    {
                                        case FrameWorkResultConstantes.DetailledMediaPlan.graphicItemType.present:
                                            if (showValue) 
                                                t.Append("<td class=\"" + presentClass + "\">" + string.Format(fp, unit.StringFormat, ((MediaPlanItem)tab[i, j]).Unit) + "</td>");
                                            else
        ...
        private static MediaPlanResultData GetHtml(object[,] tab,WebSession webSession, MediaSchedulePeriod period, string zoom, StringBuilder header, ref Int64 nbWeek, bool isExport, int idVehicle, bool isCreative) {
            ...
            if (!isExport && !isCreative) {
                if (webSession.CustomerLogin.CustormerFlagAccess(Flags.ID_SLOGAN_ACCESS_FLAG)) {
                    showVersions = true;
                        ...
                        switch(j) {
                            ...
                            case DetailledMediaPlan.L1_COLUMN_INDEX:
                                if(tab[i,j]!=null) {
                                    ...
                                    //MAJ GR : totaux par années si nécessaire
                                    for(k=1;k<=nbColYear;k++) {
                                        tmpString = WebFunctions.Units.ConvertUnitValueToString(tab[i, j + 10 + k], webSession.Unit, fp);
                            ...
                            case DetailledMediaPlan.L2_COLUMN_INDEX:
                                if(tab[i,j]!=null) {
                                    ...
                                    //MAJ GR : totaux par années si nécessaire
                                    for(k=1;k<=nbColYear;k++) {
                                        tmpString = WebFunctions.Units.ConvertUnitValueToString(tab[i,j+9+k],webSession.Unit, fp);
                                        if (!tmpString.TrimEnd(' ').Equals(""))
                            ...
                            case DetailledMediaPlan.L3_COLUMN_INDEX:
                                if(tab[i,j]!=null) {
                                    ...
                                    //MAJ GR : totaux par années si nécessaire
                                    for(k=1;k<=nbColYear;k++) {
                                        tmpString = WebFunctions.Units.ConvertUnitValueToString(tab[i, j + 8 + k], webSession.Unit, fp);
                                        if (!tmpString.TrimEnd(' ').Equals(""))
                            ...
                            case DetailledMediaPlan.L4_COLUMN_INDEX:
                                ...
                                //MAJ GR : totaux par années si nécessaire
                                for(k=1;k<=nbColYear;k++) {
                                    tmpString = WebFunctions.Units.ConvertUnitValueToString(tab[i, j + 7 + k], webSession.Unit, fp);
                                    if (!tmpString.TrimEnd(' ').Equals(""))
