...
namespace TNS.AdExpress.Web.UI.Results.APPM{
	...
	public class AnalyseFamilyInterestPlanUI{
		...
		public static string InterestFamilyPlan(WebSession webSession,IDataSource dataSource,Int64 idWave,int dateBegin,int dateEnd,Int64 idBaseTarget,Int64 idAdditionalTarget, bool excel){
			...
			html.Append("<td  class=\""+classCss2+"\" nowrap >"+WebFunctions.Units.ConvertUnitValueAndPdmToString(InterestFamilyPlanData.Rows[0]["totalBaseTargetUnit"],webSession.Unit,false, fp)+" </td>");
			html.Append("<td  class=\""+classCss2+"\" nowrap >  "+GestionWeb.GetWebWord(1684,webSession.SiteLanguage)+ " </td>");
			if(webSession.Unit==WebConstantes.CustomerSessions.Unit.grp){
				if(!excel)
                    html.Append("<td class=\"" + classCssImg + "\"><img width=1px></td>");
				html.Append("<td  class=\""+classCss2+"\" nowrap >"+WebFunctions.Units.ConvertUnitValueAndPdmToString(InterestFamilyPlanData.Rows[0]["totalAdditionalTargetUnit"],webSession.Unit,false, fp)+" </td>");
				html.Append("<td  class=\""+classCss2+"\" nowrap >"+GestionWeb.GetWebWord(1684,webSession.SiteLanguage)+ " </td>");
			}
			html.Append("</tr>");
			#endregion
...
//				html.Append("<td  class=\""+classCss3+"\" nowrap >"+Convert.ToDouble(InterestFamilyPlanData.Rows[i]["unitBase"]).ToString("# ### ##0.##")+"</td>");
				html.Append("<td  class=\""+classCss3+"\" nowrap >"+WebFunctions.Units.ConvertUnitValueAndPdmToString(InterestFamilyPlanData.Rows[i]["unitBase"],webSession.Unit,false, fp)+"</td>");
...
//					html.Append("<td  class=\""+classCss3+"\" nowrap >"+Convert.ToDouble(InterestFamilyPlanData.Rows[i]["unitSelected"]).ToString("# ### ##0.##")+"</td>");
					html.Append("<td  class=\""+classCss3+"\" nowrap >"+WebFunctions.Units.ConvertUnitValueAndPdmToString(InterestFamilyPlanData.Rows[i]["unitSelected"],webSession.Unit,false, fp)+"</td>");
