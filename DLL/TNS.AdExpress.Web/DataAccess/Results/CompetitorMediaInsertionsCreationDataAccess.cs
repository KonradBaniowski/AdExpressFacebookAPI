...
namespace TNS.AdExpress.Web.DataAccess.Results {
	...
	public class CompetitorMediaInsertionsCreationDataAccess {
		...
		public static DataSet GetData(WebSession webSession, Int64 idVehicle, Int64 idCategory, Int64 idMedia, Int64 idAdvertiser, int dateBegin, int dateEnd) {
			...
			sql.Append("select " + GetFields(VehiclesInformation.DatabaseIdToEnum(long.Parse(idVehicle.ToString())), true) + " from ( ");
			int nbUnivers = 0;
			...
			// Ordre
			sql.Append(" ) Order by " + GetOrder(VehiclesInformation.DatabaseIdToEnum(long.Parse(idVehicle.ToString())), true));
			...
			switch (idVehicle) {
				...
				case DBClassificationConstantes.Vehicles.names.internationalPress:
					return "media"
						...
						+ ", color"
                        + ", " + prefix + UnitsInformation.List[UnitsInformation.DefaultCurrency].DatabaseField + " as " + UnitsInformation.List[UnitsInformation.DefaultCurrency].Id.ToString()
						+ ", location"
						+ ", " + prefix + "visual "
