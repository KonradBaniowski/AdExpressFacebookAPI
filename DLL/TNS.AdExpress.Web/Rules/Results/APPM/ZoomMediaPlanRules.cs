...
namespace TNS.AdExpress.Web.Rules.Results.APPM
{
	...
	/// </summary>
	public class ZoomMediaPlanRules
	{
...
/*			
			...
			DataSet ds;
//			ds=MediaPlanAlertDataAccess.GetPluriMediaDataset(webSession, periodBeginning, periodEnd); //Old version
			ds=TNS.AdExpress.Web.DataAccess.Results.APPM.MediaPlanDataAccess.GetData(webSession,dataSource,int.Parse(beginningPeriod),int.Parse(endPeriod),idBaseTarget,idAdditionalTarget);
			#endregion
			...
			int FIRST_PERIOD_INDEX = FrameWorkConstantes.Results.MediaPlanAPPM.FIRST_PEDIOD_INDEX;
...
//			}
			...
			// Nombre de colonne
			int nbCol=periodItemsList.Count+FrameWorkConstantes.Results.MediaPlanAPPM.FIRST_PEDIOD_INDEX+3; // 3==budget+pages+grp
			// Tableau de résultat
...
//			tab[currentTotalIndex,FrameWorkConstantes.Results.MediaPlanAlert.TOTAL_COLUMN_INDEX]=(double)0.0;
			...
			foreach(DataRow currentRow in dt.Rows) {
				...
				if(oldIdVehicle!=(Int64)currentRow["id_vehicle"]){
					...
					currentLineIndex++;
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.VEHICLE_COLUMN_INDEX]=currentRow["vehicle"].ToString();
//					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.TOTAL_COLUMN_INDEX]=(double)0.0;
...
//					//fin MAJ
					...
					oldIdVehicle=(Int64)currentRow["id_vehicle"];
					numberOflineToAdd++;
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.CATEGORY_COLUMN_INDEX]=null;
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.MEDIA_COLUMN_INDEX]=null;
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.ID_MEDIA_COLUMN_INDEX]=null;
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.PERIODICITY_COLUMN_INDEX]=null;										

					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.ID_VEHICLE_COLUMN_INDEX]=(Int64)currentRow["id_vehicle"];
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.ID_CATEGORY_COUMN_INDEX]=(Int64)currentRow["id_category"];
					tab[currentLineIndex,FrameWorkConstantes.Results.MediaPlanAPPM.ID_MEDIA_COLUMN_INDEX]=(Int64)currentRow["id_media"];
				}
				// Nouvelle Category
				if(oldIdCategory!=(Int64)currentRow["id_category"]){
