    ...
    public class EngineMediaStrategy:Engine
    {
				...
				*/
                if (dtTotalMarketOrSector != null && dtTotalMarketOrSector.Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(VehicleAccessList)
                        &&
                        (CstDbClassif.Vehicles.names.plurimedia ==
                         VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                         ||
                         CstDbClassif.Vehicles.names.PlurimediaWithoutMms ==
                         VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                         ||
                         CstDbClassif.Vehicles.names.plurimediaOnline ==
                         VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                         ||
                         CstDbClassif.Vehicles.names.plurimediaOffline ==
                         VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                         ||
                         CstDbClassif.Vehicles.names.plurimediaWithSearch ==
                         VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                        ))
                    {
                        ...
                        {
                            ComputeInvestPdmEvol(dtTotalMarketOrSector, ref TotalMarketOrSectorInvest,
                                ref TotalMarketOrSectorInvest, ref TotalMarketOrSectorInvest_N1, ref tempPDM,
                                ref tempEvol, "Sum(total_N)", "Sum(total_N1)", "", _session.ComparativeStudy);
                            ...
                            {
                                string webWord = CstDbClassif.Vehicles.names.PlurimediaWithoutMms ==
                                                 VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                                    ? GestionWeb.GetWebWord(3020, _session.SiteLanguage)
                                    : GestionWeb.GetWebWord(210, _session.SiteLanguage);
                                tab = FillTabInvestPdmEvol(tab, indexTabRow, VehicleAccessList, webWord, true,
                                    TotalMarketOrSectorInvest, "100", tempEvol, _session.ComparaisonCriterion,
                                    CstResult.MediaStrategy.InvestmentType.total,
                                    CstPreformatedDetail.PreformatedMediaDetails.vehicle);
                                increment = true;
                            }
                            tab = FillTabFisrtElmt(tab, comparisonCriterion, TotalMarketOrSectorInvest, ref indexTabRow,
                                increment);
                            increment = false;
                        ...
                        if (dtTotalUniverse != null && dtTotalUniverse.Columns.Contains("total_N") && !hasTotalUniv)
                        {
                            ComputeInvestPdmEvol(dtTotalUniverse, ref TotalUnivInvest, ref TotalUnivInvest,
                                ref TotalUnivInvest_N1, ref tempPDM, ref tempEvol, "Sum(total_N)", "Sum(total_N1)", "",
                            ...
                            if (FctUtilities.CheckedText.IsNotEmpty(TotalUnivInvest.Trim()))
                            {
                                string webWord = CstDbClassif.Vehicles.names.PlurimediaWithoutMms ==
                                    ...
                                    : GestionWeb.GetWebWord(210, _session.SiteLanguage);
                                tab = FillTabInvestPdmEvol(tab, indexTabRow, VehicleAccessList, webWord, true,
                                    TotalUnivInvest, "100", tempEvol, CstComparaisonCriterion.universTotal,
                                    CstResult.MediaStrategy.InvestmentType.total,
                                    CstPreformatedDetail.PreformatedMediaDetails.vehicle);
                                increment = true;
                            }
                            tab = FillTabFisrtElmt(tab, CstComparaisonCriterion.universTotal, TotalUnivInvest,
                                ref indexTabRow, increment);
                        ...
                        if (FctUtilities.CheckedText.IsNotEmpty(AdvertiserAccessList) && dtAdvertiser != null &&
                            dtAdvertiser.Rows.Count > 0 && !hasAdvertiser)
                        {
                            //Pour chaque ligne  TOTAL annonceur de référence ou concurrent on récupère les données							
                            FillAdvertisers(tab, dtAdvertiser, dtTotalMarketOrSector, "Sum(total_N)", "Sum(total_N1)",
                                "", inTotUnivAdvertAlreadyUsedArr, ref hPdmTotAdvert, ref hPdmTotAdvert,
                                ref AdvertiserTotalInvest, ref AdvertiserTotalInvest_N1, ref indexTabRow,
                                VehicleAccessList, idVehicle, Vehicle, true, ref hasAdvertiser);
                        }
                            ...
                            if (!inTotMarketOrSectorVehicleAlreadyUsedArr.Contains(idVehicle))
                            {
                                if (dtTotalMarketOrSector.Columns.Contains("total_N"))
                                {
                                    ...
                                    if (CstDbClassif.Vehicles.names.plurimedia == VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                                        ...
                                        || CstDbClassif.Vehicles.names.plurimediaOnline == VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                                        || CstDbClassif.Vehicles.names.plurimediaOffline == VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList))
                                        || CstDbClassif.Vehicles.names.plurimediaWithSearch == VehiclesInformation.DatabaseIdToEnum(Int64.Parse(VehicleAccessList)))
            ...
            if (dtTotalMarketOrSector != null && dtTotalMarketOrSector.Rows.Count > 0)
            {
                    ...
                    )
                {
					...
					if (dtTotalUniverse != null && dtTotalUniverse.Columns.Contains("total_N") && !hasTotalUniv)
                    {
						...
						if (dtTotalUniverse!=null && dtTotalUniverse.Columns.Contains("total_N"))
                    ...
                    if (dtTotalUniverse != null && dtTotalUniverse.Columns.Contains("id_category") && dtTotalUniverse.Columns.Contains("category"))
                    {
                        ...
                        if (!inTotUnivCategoryAlreadyUsedArr.Contains(idCategory))
                        {
                            ...
                            if (FctUtilities.CheckedText.IsNotEmpty(AdvertiserAccessList))
                            {
                                ...
                                {
                                    //Pour chaque ligne  annonceur de référence ou concurrent on récupère les données
									if (dtTotalUniverse!= null && dtTotalUniverse.Columns.Contains("id_media"))
                                    {
                                        //si le niveau support est le plus bas alors vider la collection des annonceurs anciennement traités
                                        if (start == 1 && ((int.Parse(idCategory) != int.Parse(OldIdCategory)) || (int.Parse(idMedia) != int.Parse(OldIdMedia))))
                                        {
                                    ...
                                    if ((int.Parse(idCategory) != int.Parse(OldIdCategory)) || (int.Parse(idVehicle) != int.Parse(OldIdVehicle)))
                                    {
                                        ...
                                        foreach (DataRow currentAdvertRow in dtAdvertiser.Rows)
                                        {
                                            ...
                                            if (dtAdvertiser.Columns.Contains("total_N"))
                                            {
                                                ...
                                                if (!inAdvertCategoryAlreadyUsedArr.Contains(idCategory) && (dtTotalUniverse==null || !dtTotalUniverse.Columns.Contains("id_media")))
                                                {
                                                    #region  catégorie est le niveau le plus bas
                                                    strExpr = "id_vehicle = " + idVehicle + " AND id_category = " + idCategory;
                                                    strSort = "id_category ASC";
                                                    ...
                                                    if (foundRows != null && foundRows.Length > 0)
                                                    {
                                                        for (int n = 0; n < foundRows.Length; n++)
                                                        {
															...
															AdvertiserInvestByCat = foundRows[n]["total_N"].ToString();
                                                            //Insertion des données dans la ligne courante pour un annonceur et par catégorie
                                                            if (FctUtilities.CheckedText.IsNotEmpty(AdvertiserInvestByCat.Trim()))
        ...
        protected object[,] FillAdvertisers(object[,] tab, DataTable dtTotalMarketOrSector, DataRow[] foundRows, ref string AdvertiserInvest, ref string AdvertiserInvest_N1, ref Hashtable hPdmParentAdvert, ref Hashtable hPdmChildAdvert, ref int indexTabRow, string idVehicle, string Vehicle, string idCategory, string Category, string idMedia, string Media, CstPreformatedDetail.PreformatedMediaDetails preformatedMediaDetails, bool isPluri)
        {
            ...
            for (int n = 0; n < foundRows.Length; n++)
            {
                ...
                if(FctUtilities.CheckedText.IsNotEmpty(foundRows[n]["total_N"].ToString()))
					AdvertiserInvest = foundRows[n]["total_N"].ToString(); // String.Format("{0:### ### ### ### ##0.##}", (double.Parse(foundRows[n]["total_N"].ToString()) / (double)1000));															
                ...
                if (_session.ComparativeStudy && dtTotalMarketOrSector.Columns.Contains("total_N") && dtTotalMarketOrSector.Columns.Contains("total_N1"))
                {
					...
					if (FctUtilities.CheckedText.IsNotEmpty(foundRows[n]["total_N1"].ToString()))
						AdvertiserInvest_N1 = foundRows[n]["total_N1"].ToString(); //String.Format("{0:### ### ### ### ##0.##}", (double.Parse(foundRows[n]["total_N1"].ToString()) / (double)1000));
                        ...
                        switch (preformatedMediaDetail)
                        {
                            ...
                            case CstPreformatedDetail.PreformatedMediaDetails.vehicleCategory:
								if (_session.ComparaisonCriterion == CstComparaisonCriterion.marketTotal) {
									...
									tab[indexTabRow, TOTAL_SECTOR_CATEGORY_INVEST_COLUMN_INDEX] = Invest;									
								}
                                break;
		...
		/// <param name="allIdCategories">List of all id categories</param>
