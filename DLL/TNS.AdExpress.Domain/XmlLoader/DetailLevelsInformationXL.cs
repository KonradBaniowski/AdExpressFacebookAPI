...
namespace TNS.AdExpress.Domain.XmlLoader {
	...
	public class DetailLevelsInformationXL {
		...
		public static Hashtable Load(IDataSource source){
			...
			try{
				...
				source.Close();
				throw(new DetailLevelsInformationXLException("Impossible to load the GenericDetailLevel XML file",err));
			}
		}
		...
		public static ArrayList LoadDetailLevel(IDataSource source){
			...
			try{
				...
				source.Close();
				throw(new DetailLevelsInformationXLException("Impossible to load the MediaPlanInsertionConfiguration XML file",err));
			}
		}
	}
}
