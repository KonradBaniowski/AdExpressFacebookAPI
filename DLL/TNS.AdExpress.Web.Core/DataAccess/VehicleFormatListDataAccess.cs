...
namespace TNS.AdExpress.Web.Core.DataAccess {
    ...
    class VehicleFormatListDataAccess {
        ...
        /// <summary>
        /// Method used to extract the list of banners format according to the specified vehicle
        /// </summary>
        /// <param name="formatTable">Format Table</param>
        /// <param name="dataLanguage">Data language</param>
        /// <param name="excludedFormats">Excluded Fomrats</param>
        /// <returns>Format List</returns>
        public static DataSet GetAllData(TableIds formatTable, int dataLanguage, string excludedFormats) {
            ...
            sql.AppendFormat("Select distinct {0}.id_{1}, {0}.{1} from {2} where {0}.id_{1} not in ({5}) and {0}.id_language = {3} and {0}.activation <{4} order by {0}.{1} asc"
                , formatBannersTable.Prefix
                , formatBannersTable.Label
                , formatBannersTable.SqlWithPrefix
                , dataLanguage
