...
namespace TNS.AdExpress.Web.Core.DataAccess.ClassificationList
{
    ...
    public class UniversListDataAccess
    {
		...
		/// <param name="idUniverse">Identifiant de l'univers</param>
		/// <param name="webSession">Session du client</param>
        /// <param name="medias">media list</param>
		/// <returns>Retourne l'objet récupéré ou null si il y a eu un problème non géré</returns>
		public static Object GetTreeNodeUniverseWithMedia(Int64 idUniverse, WebSession webSession,out List<long> medias)
        {
             medias = new List<long>();
            ...
            try
            {
                ...
                block.Append(" BEGIN  ");
                block.AppendFormat(" SELECT blob_universe_client, FILTER INTO :1, :2  FROM {0}", universeTable.Sql);
                block.AppendFormat(" WHERE id_universe_client = {0} ;", idUniverse);
                block.Append(" END; ");

                //string block = " SELECT blob_universe_client , FILTER FROM " + universeTable.Sql + " WHERE id_universe_client = " + idUniverse + ";";

                sqlCommand = new OracleCommand(block.ToString());
                sqlCommand.Connection = cnx;
                sqlCommand.CommandType = CommandType.Text;
                ////Initialize parametres
                OracleParameter param = sqlCommand.Parameters.Add("blobfromdb1", OracleDbType.Blob);
                ...
                OracleParameter param2 = sqlCommand.Parameters.Add("fil", OracleDbType.Varchar2);
