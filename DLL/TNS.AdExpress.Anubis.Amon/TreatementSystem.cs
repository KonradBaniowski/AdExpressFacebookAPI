...
using TNS.AdExpress.Anubis.Amon.BusinessFacade;
...
namespace TNS.AdExpress.Anubis.Amon
{
    ...
    : IPlugin
    {
        ...
        /// </summary>
        private AmonConfig _amonConfig;
        /// <summary>
        /// Theme
        /// </summary>
        ...
        {
            return ("Amon (Rolex Schedule PDF Generator)");
        ...
        public void Treatement(string configurationFilePath, IDataSource dataSource, Int64 navSessionId)
        {
            try
            {
                ...
                try
                {
                    ...
                    _dataAccess = (IStaticNavSessionDAL)AppDomain.CurrentDomain.CreateInstanceFromAndUnwrap(AppDomain.CurrentDomain.BaseDirectory + cl.AssemblyName, cl.Class, false, BindingFlags.CreateInstance
                        | BindingFlags.Instance | BindingFlags.Public, null, parameter, null, null);
                }
                catch (Exception e)
                {
                    _dataAccess = null;
                    throw new AmonPdfException("Impossible to Create Instance Of Layer IStaticNavSessionDAL ", e);
                }
                #endregion
                ...
                {
                    throw new AmonPdfException("Impossible de lancer le traitement d'un job", new ArgumentNullException("Le nom du fichier de configuration est null."));
                }
                if (configurationFilePath.Length == 0)
                {
                    throw new AmonPdfException("Impossible de lancer le traitement d'un job", new ArgumentException("Le nom du fichier de configuration est vide."));
                }
                #endregion

                #region Initialize Amon
                try
                {
                    _amonConfig = new AmonConfig(new XmlReaderDataSource(configurationFilePath));
                }
                catch (System.Exception err)
                {
                    throw new AmonPdfException("Impossible de lancer le traitement d'un job <== impossible de charger le fichier de configuration", err);
                }
                #endregion
                ...
                {
                    throw new AmonPdfException("Error for load session", err);
                }
                #endregion
                ...
                try
                {
                    ...
                    if (File.Exists(_amonConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml"))
                    {
                        pathFileTheme = _amonConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml";
                    }
                    else if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + _amonConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml"))
                    {
                        pathFileTheme = AppDomain.CurrentDomain.BaseDirectory + _amonConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml";
                    }
                    else
                    {
                        pathFileTheme = _amonConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml";
                ...
                {
                    throw new AmonPdfException(string.Format("File of theme not found ! (in Plugin Amon in TreatmentSystem class) - Path : '{0}'", pathFileTheme), err);
                }
                #endregion
            ...
            _myThread = new Thread(myThreadStart) {Name = "Amon (Rolex Schedule PDF Generator)"};
            _myThread.Start();
        }
        ...
        /// <summary>
        /// Generate the PDF for Amon plug-in
        /// </summary>
        private void ComputeTreatement()
        {
           AmonPdfSystem pdf = null;
            try
            {
                ...
                pdf = new AmonPdfSystem(_dataSource, _amonConfig, rqDetails, _webSession, _theme);
                ...
                PluginInformation pluginInformation = PluginConfiguration.GetPluginInformation(PluginType.Amon);
#if (!DEBUG)
