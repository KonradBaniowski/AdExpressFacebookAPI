...
namespace TNS.AdExpress.Anubis.Shou
{
	...
	public class TreatementSystem : IPlugin {
		...
		public void Treatement(string configurationFilePath,IDataSource dataSource,Int64 navSessionId) {
            ...
            try {
                _theme = new Theme(new XmlReaderDataSource(AppDomain.CurrentDomain.BaseDirectory + _shouConfig.ThemePath + @"\App_Themes\" + WebApplicationParameters.Themes[((ProofDetail)_dataAccess.LoadData(_navSessionId)).CustomerWebSession.SiteLanguage].Name + @"\" + "Styles.xml"));
            }
            catch (System.Exception err) {
		...
		private void ComputeTreatement() {
			...
			try {
                ...
                pdf = new ShouPdfSystem(_dataSource, _shouConfig, rqDetails, (ProofDetail)_dataAccess.LoadData(_navSessionId), _theme);
