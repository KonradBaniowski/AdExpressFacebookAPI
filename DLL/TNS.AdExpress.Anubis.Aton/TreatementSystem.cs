...
namespace TNS.AdExpress.Anubis.Aton{
	...
	public class TreatementSystem:TNS.AdExpress.Anubis.BusinessFacade.IPlugin{
		...
		public void Treatement(string configurationFilePath,IDataSource dataSource,Int64 navSessionId){
			...
			try{
				_atonConfig=new AtonConfig(new XmlReaderDataSource(AppDomain.CurrentDomain.BaseDirectory+configurationFilePath));
			}
		...
		private void ComputeTreatement(){
			...
			try{
				...
				try{
					TNS.AdExpress.Anubis.Common.Functions.CleanWorkDirectory(pdf.GetWorkDirectory());
				}
