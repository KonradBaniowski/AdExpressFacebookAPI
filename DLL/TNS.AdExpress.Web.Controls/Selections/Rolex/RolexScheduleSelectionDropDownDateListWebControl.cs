...
namespace TNS.AdExpress.Web.Controls.Selections.Rolex
{
    [DefaultProperty("Text")]
    [ToolboxData("<{0}:RolexSelectionDropDownDateListWebControl runat=server></{0}:RolexSelectionDropDownDateListWebControl>")]
    public class RolexScheduleSelectionDropDownDateListWebControl : AjaxSelectionBaseWebControl
    {
        ...
        protected override string GetAjaxEventScript()
        {
            ...
            js.Append("\r\n\t isLoaded_RolexScheduleContainerWebControl1ResultFilter_Dates = false;");
        ...
        public void SetDate(string idSession, string periodType, string datePersonalize, AjaxPro.JavaScriptObject resultParameters, AjaxPro.JavaScriptObject styleParameters)
        {
            ...
            try
            {
                ...
                {
                    _webSession.SetDates(periodTypes);
                }
                else
                {
                    _webSession.SetDates(TNS.FrameWork.Date.DateString.YYYYMMDDToDateTime(datePersonalize.Split('_')[0])
        ...
        protected override string GetAjaxHTML()
        {
            ...
            foreach (DateConfiguration cVpDateConfiguration in WebApplicationParameters.RolexDateConfigurations.VpDateConfigurationList)
            {
            ...
            if (_webSession.PeriodType == CustomerSessions.Period.Type.personalize)
            {
