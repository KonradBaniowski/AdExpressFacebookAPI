...
namespace TNS.AdExpress.Web.Controls.Selections.Russia
{
    ...
    public class SectorAnalysisVehicleSelectionWebControl : System.Web.UI.WebControls.CheckBoxList
    {
            ...
            if (!Page.IsPostBack)
            {
                if (webSession != null)
                {
                    try
                    {
                        ...
                        DataSet ds = classficationDAL.GetRecapDetailMedia();
                    ...
                    {
                        throw (new WebControlInitializationException("Impossible to get data for Product class analysis Vehicle Selection WebControl "));
                    }
                }
        ...
        protected override void OnPreRender(EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (vehicleInfo != null)
                {
                    ...
                    this.Items.Add(new System.Web.UI.WebControls.ListItem(GestionWeb.GetWebWord(210, webSession.DataLanguage), "vh_" + vehicleInfo.DatabaseId.ToString()));
                ...
                foreach (DataRow currentRow in dtVehicle.Rows)
                {
                    ...
                    if ((((IdCategory = Int64.Parse(currentRow["id_region"].ToString())) != oldCategory) || isNewVehicle ) && showCategory(Int64.Parse(currentRow["id_vehicle"].ToString())))
                    {
                        oldCategory = IdCategory;
                        this.Items.Add(new System.Web.UI.WebControls.ListItem(currentRow["region"].ToString(),"ct_" + Int64.Parse(currentRow["id_region"].ToString())));
                    }
                    if (showMedia(Int64.Parse(currentRow["id_vehicle"].ToString())))
                    {
                        this.Items.Add(new System.Web.UI.WebControls.ListItem(currentRow["media"].ToString(), "md_" + Int64.Parse(currentRow["id_media"].ToString())));
                    }
            ...
            if (dtVehicle != null && dtVehicle.Rows.Count > 0)
            {
                ...
                if (_currentVehicleList != null && _currentVehicleList.Count > 0)
                {
                    ...
                    for (int v = 0; v < _currentVehicleList.Count; v++)
                    {
                        ...
                        if (vehicleInfo.DatabaseId != v) vhlist += "vhDiv" + _currentVehicleList[v].ToString() + ",";
                    }
                }
                if (!string.IsNullOrEmpty(vhlist)) vhlist = vhlist.Substring(0, vhlist.Length - 1);
                ...
                foreach (DataRow currentRow in dtVehicle.Rows)
                {
                    ...
                    idVehicle = Convert.ToInt64(currentRow["id_vehicle"].ToString());
                    idCategory = Convert.ToInt64(currentRow["id_region"].ToString());
                    ...
                    if (idVehicle != idVehicleOld)
                    {
                        ...
                        if (OpenVehicleDiv(idVehicle))
                        {
                            t.Append("<tr><td class=\"roll04\"><a href=\"javascript: SelectExclusiveAllChilds('vh_" + idVehicle + "','" + VehicleIds + "','vh_" + idVehicle + "','vh_','ct_')\" title=\"" + GestionWeb.GetWebWord(2800, webSession.SiteLanguage) + "\" class=\"roll04\">&nbsp;&nbsp;" + GestionWeb.GetWebWord(2800, webSession.SiteLanguage) + "</td></tr>");
                            t.Append("<TR><TD>");
                        }
                        i++;
                    }
                    ...
                    #region New category
                    if (idCategory != idCategoryOld && showMediaBranchItem(TableName.region, idVehicle))
                    {
                        numColumn = 0;
                        if (startCategory == -1) t.Append("<tr><td ><div style=\"MARGIN-LEFT: 5px; width=100%;\" class=\"violetBackGroundV3\"  id=\"" + idVehicle.ToString() + "ct_" + idCategory + "\"><table class=\"violetBackGroundV3 txtViolet11Bold\"  cellpadding=0 cellspacing=0 border=\"0\" width=\"100%\">");
                        else t.Append("<tr><td><div style=\"MARGIN-LEFT: 5px; width=100%;\" class=\"violetBackGroundV3\"  id=\""+idVehicle.ToString()+"ct_" + idCategory + "\"><table class=\"violetBackGroundV3 violetBorderTop txtViolet11Bold\"  cellpadding=0 cellspacing=0 border=\"0\" width=\"100%\">");
                        ...
                        // checkbox category 
                        t.Append("<input type=checkbox name=\"SectorAnalysisVehicleSelectionWebControl1$" + i + "\" id=\"SectorAnalysisVehicleSelectionWebControl1_" + i + "\" onClick=\"CheckAllChilds('" + idVehicle.ToString() + "ct_" + idCategory + "','" + VehicleIds + "','vh_" + idVehicle + "')\" value=\"ct_" + idCategory + "\">");
                        t.Append("\n</td>\n");
                        //Label category
                        t.Append("<td width=100% valign=\"middle\" height=\"10\" onClick=\"javascript : DivDisplayer('" + idVehicle.ToString() +"_" +  idCategory + "');\" align=\"left\"  ");
                        if (showMediaBranchItem(TableName.media, idVehicle))
                        {
                            t.Append(">&nbsp;" + currentRow["region"].ToString() + "</td>");
                            t.Append("<td valign=\"baseline\"  onClick=\"javascript : DivDisplayer('"+idVehicle+"_" + idCategory + "');\"   ");
                            t.Append(">&nbsp;<IMG height=\"15\" src=\"/App_Themes/" + themeName + "/images/Common/button/bt_arrow_down.gif\" width=\"15\" align=\"right\"></td>");
                        }
                        else t.Append(" width=\"15\" colspan=\"2\" >&nbsp;" + currentRow["region"].ToString() + "</td>");

                        t.Append("</tr><tr><td colspan=\"3\"><DIV id=\""+idVehicle+"_" + idCategory + "\" ><table cellpadding=0 cellspacing=0 border=\"0\" width=100%>");
                        if (showMediaBranchItem(TableName.media, idVehicle))
                        {
                            t.Append("<table cellpadding=0 cellspacing=0 border=\"0\" width=100% class=\"mediumPurple1 whiteTopBorder txtViolet10\">");
                            t.Append("<tr><td colspan=\"3\" class=\"roll04\" ><a href=\"javascript: SelectExclusiveAllChilds('"+idVehicle.ToString() +"ct_" + idCategory + "','" + VehicleIds + "','vh_" + idVehicle + "','vh_','ct_')\" title=\"" + GestionWeb.GetWebWord(1066, webSession.SiteLanguage) + "\" class=\"roll04\">&nbsp;" + GestionWeb.GetWebWord(1066, webSession.SiteLanguage) + "</a></td></tr>");
                        }
                        i++;
                    }
                    #endregion
                    ...
                    if (showMediaBranchItem(TableName.media, idVehicle))
                    {
                        if (numColumn == 0)
                        {
                            ...
                            t.Append("<input type=\"checkbox\" " + checkBox + " name=\"SectorAnalysisVehicleSelectionWebControl1$" + i + "\" id=\"SectorAnalysisVehicleSelectionWebControl1_" + i + "\" value=\"md_" + currentRow["id_media"] + "\" onClick=\"CheckAllChilds('md_" + currentRow["id_media"] + "','" + VehicleIds + "','vh_" + idVehicle + "')\">" + currentRow["media"].ToString());
                            t.Append("</td>");
                            numColumn++;
                            i++;
                        }
                        ...
                        {
                            t.Append("<td  width=\"33%\">");
                            t.Append("<input type=\"checkbox\" name=\"SectorAnalysisVehicleSelectionWebControl1$" + i + "\" onClick=\"CheckAllChilds('md_" + currentRow["id_media"] + "','" + VehicleIds + "','vh_" + idVehicle + "')\"  value=\"md_" + currentRow["id_media"] + "\" id=\"SectorAnalysisVehicleSelectionWebControl1_" + i + "\" >" + currentRow["media"].ToString());
                            t.Append("</td></tr>");
                            numColumn = 0;
                            i++;
                        }
                ...
                //Close Category				
                if (startCategory == 0 && showMediaBranchItem(TableName.region, idVehicle))
                {	//Fermeture support
                    if (showMedia(idVehicle))
                    {
                        if (numColumn != 0) t.Append("</tr>");
        ...
        private bool showMediaBranchItem(DBConstantesClassification.Table.name mediaBranchItem, Int64 idVehicle)
        {
            ...
            switch (mediaBranchItem)
            {
                case DBConstantesClassification.Table.name.vehicle:
                    return (true);
                case DBConstantesClassification.Table.name.region:
        ...
        {
            VehicleInformation vehicleInfo = VehiclesInformation.Get(idVehicle);
            return (vehicleInfo.AllowedRecapMediaLevelItemsEnumList != null && vehicleInfo.AllowedRecapMediaLevelItemsEnumList.Contains(DetailLevelItemInformation.Levels.region));
