    ...
    public class MediaSelectionWebControl : System.Web.UI.WebControls.CheckBoxList
    {
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            if (dsListMedia != null && dsListMedia.Tables[0].Rows.Count > 0)
            {
                ...
                VehicleIds = GetVehicleIds(dsListMedia);
        ...
        protected string GetVehicleIds(DataSet ds)
        {
            string res = "";
            const string vhSeparator = "-";
            var oldVehiclesIds = new List<long>();
            bool first = true;
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                long idVehicle = Convert.ToInt64(dr["id_vehicle"]);
                if (!oldVehiclesIds.Contains(idVehicle))
                {
                    res += (first?"" : vhSeparator )+ "vh_" + idVehicle.ToString();
                    oldVehiclesIds.Add(idVehicle);
                    first = false;
                }
            }
            return res;
