								...
								if(currentRow["sloganFile"]!=null && currentRow["sloganFile"].ToString().Length > 0){
									...
									switch ((DBConstantesClassification.Vehicles.names)int.Parse(idVehicle.ToString())) {
										...
										case TNS.AdExpress.Constantes.Classification.DB.Vehicles.names.internationalPress:
											...
											string pathWebImagette = WebConstantes.CreationServerPathes.IMAGES + "/" + currentRow["id_media"].ToString() + "/" + currentRow["date_media_num"].ToString() + "/imagette/";
											pathWeb = WebConstantes.CreationServerPathes.IMAGES + "/" + currentRow["id_media"].ToString() + "/" + currentRow["date_media_num"].ToString() + "/";

											string pathImagette = WebConstantes.CreationServerPathes.LOCAL_PATH_IMAGE + currentRow["id_media"].ToString() + @"\" + currentRow["date_media_num"].ToString() + @"\imagette\";
											// Pour test en localhost :
											//										string pathImagette="\\\\localhost\\ImagesPresse\\"+currentRow["id_media"].ToString()+"\\"+currentRow["date_media_num"].ToString()+"\\imagette\\";
											if (fileList != null) {
