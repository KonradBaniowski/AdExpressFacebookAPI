...
namespace TNS.AdExpress.Web.Controls.Headers{
	...
	public class ActualitiesWebControl : System.Web.UI.WebControls.WebControl{
		...
		protected override void OnPreRender(EventArgs e) {
			...
			//bool success = rss.DownloadRss("http://assouan/test2.xml");
#if DEBUG
			_rssFileUrl = "http://frmitch-ws003/Rss/" + _languageId.ToString() + "/" + _rssFileName;
			...
			bool success = rss.DownloadRss(_rssFileUrl);
			...
			if (success) {
				...
				if (numItems > 0) {
					for (int i = 0; i < numItems; i++) {
						...
						//_items.Add("<font "+((_rssTitleCss.Length>0)?" class=\""+ _rssTitleCss+"\"":"")+">&raquo;&nbsp;"+item.GetString("title")+" : </font><font "+((_rssDescriptionCss.Length>0)?" class=\""+ _rssDescriptionCss+"\"":"")+">"+item.GetString("description")+"</font>");
						_items.Add("<font " + ((_rssDescriptionCss.Length > 0) ? " class=\"" + _rssDescriptionCss + "\"" : "") + ">&raquo;&nbsp;<font " + ((_rssTitleCss.Length > 0) ? " class=\"" + _rssTitleCss + "\"" : "") + ">" + item.GetString("title") + " : </font>" + item.GetString("description") + "</font>");
					}
				}
			}
			base.OnPreRender(e);
