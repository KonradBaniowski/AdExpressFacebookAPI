...
namespace TNS.AdExpress.Web.Controls.Headers
{
    ...
    public class SubPeriodSelectionWebControl : WebControl
    {
        ...
        /// </summary>
        private const string IMAGE_PATTERN_SELECTED = "/Images/{0}/GlobalCalendar/{1}s.gif";
        /// <summary>
        /// Pattern to determine the path to the subperiod visual
        /// </summary>
        private const string IMAGE_PATTERN = "/Images/{0}/GlobalCalendar/{1}.gif";
        /// <summary>
        /// Pattern to determine the path to the subperiod visual when mouse is over
        /// </summary>
        private const string IMAGE_PATTERN_OVER = "/Images/{0}/GlobalCalendar/{1}r.gif";
        /// <summary>
        ...
        private const string COMPONENT_BACK_GROUND = "#D0C8DA";
        ...
        public string RenderContents()
        {
            ...
            if (_zoom.Length > 0 && _isZoomEnabled && (_zoom != periodBegin || _zoom != periodEnd))
            {
                ...
                sb.AppendFormat("\r\n\t\tdocument.getElementById('img_{0}_'+index).src = tab_periodImage_over_{0}[index];"
                    , this.ID
                    , string.Format(IMAGE_PATTERN_OVER, _webSession.SiteLanguage, "index"));
                ...
                #region Design
                sb.AppendFormat("<table border=0 cellspacing=0 cellpadding=0 bgcolor=\"{0}\" width=\"100%\">", COMPONENT_BACK_GROUND);
                ...
                if (tmpSb.Length > 0)
                {
                    ...
                    if (labBegin != labEnd)
                    {
                        sb.AppendFormat("\r\n<td width=\"145\" nowrap class=\"{5}\" id=\"periodLabel_{4}\" valign=\"top\">{0} {1} {2} {3}</td>",
                    ...
                    {
                        sb.AppendFormat("\r\n<td width=\"145\" nowrap class=\"{1}\" id=\"periodLabel_{2}\" valign=\"top\">{0}</td>"
                            , labBegin
                            , FONT_PERIOD
                            , this.ID);
                    }
                    ...
                    sb.AppendFormat("<div id=\"scrollContainer_{1}\" width=\"{3}\" class=\"{0}\"><div width=\"{4}\" id=\"scrollTrack_{1}\" class=\"{2}\" style=\"cursor:hand;\"></div></div></td></tr>",
                    ...
                    sb.AppendFormat("\r\n\t\t\tvar posInit_{0} = Math.max(0,parseInt(parseInt(theScrollContent_{0}.offsetWidth-theScrollContentContainer_{0}.width) * {1} / {2}) * ((theScrollContentContainer_{0}.width-theScrollTrack_{0}.width)/theScrollContentContainer_{0}.width));", this.ID, periodIndex, i);
        ...
        private void AppendPeriod(WebCst.CustomerSessions.Period.Type periodType, string currentPeriod, int i, StringBuilder sb, StringBuilder tmpSb, ref int periodIndex, string globalDateBegin, string globalDateEnd, ref string labBegin, ref string labEnd)
        {
            ...
            sb.AppendFormat("\r\ntab_periodImage_{0}[{1}] = '{2}';",
                this.ID, i,
                string.Format(IMAGE_PATTERN, _webSession.SiteLanguage, Int32.Parse(currentPeriod.Substring(4, 2))));
            sb.AppendFormat("\r\ntab_periodImage_selected_{0}[{1}] = '{2}';",
                this.ID, i,
                string.Format(IMAGE_PATTERN_SELECTED, _webSession.SiteLanguage, Int32.Parse(currentPeriod.Substring(4, 2))));
            sb.AppendFormat("\r\ntab_periodImage_over_{0}[{1}] = '{2}';",
                this.ID, i,
                string.Format(IMAGE_PATTERN_OVER, _webSession.SiteLanguage, Int32.Parse(currentPeriod.Substring(4, 2))));
            if (_zoom == currentPeriod)
            {
                //style=\"display:none\" 
                tmpSb.AppendFormat("<img id=img_{2}_{3} src=\"" + IMAGE_PATTERN_SELECTED + "\" onMouseOver=\"javascript:PeriodMouseOver_{2}({3});\" onMouseOut=\"javascript:PeriodMouseOut_{2}({3});\" style=\"cursor:hand;\" onclick=\"javascript:PeriodSelect_{2}({3});\"/>"
                    , _webSession.SiteLanguage
            ...
            {
                tmpSb.AppendFormat("<img id=img_{2}_{3} src=\"" + IMAGE_PATTERN + "\" onMouseOver=\"javascript:PeriodMouseOver_{2}({3});\" onMouseOut=\"javascript:PeriodMouseOut_{2}({3});\" style=\"cursor:hand;\" onclick=\"javascript:PeriodSelect_{2}({3});\"/>"
                    , _webSession.SiteLanguage
                    , Int32.Parse(currentPeriod.Substring(4, 2))
                    , this.ID, i);
