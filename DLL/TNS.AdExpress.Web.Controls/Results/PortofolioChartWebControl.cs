...
namespace TNS.AdExpress.Web.Controls.Results {
	...
	public class PortofolioChartWebControl : Chart {
		...
		protected virtual void SetDesignMode() {
            try
            {
                ...
                else
                {
                    ...
                    if (!valuesListSize.ContainsKey(oldUnitId)) valuesListSize.Add(oldUnitId, nbItem);
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        if (oldUnitId != long.Parse(dr["idUnit"].ToString()) && !start)
                        {
                            if (xValues != null && xValues.Length > 0 && yValues != null && yValues.Length > 0)
                            {
                                ...
                                InitializeComponent(chartArea, _typeFlash, _webSession, _defaultBorderLineColor, _chartBorderLineColor, _titleColor, indexTitle, numberofChart, ref posY);
            ...
            {
                throw new Exceptions.PortofolioChartWebControlException(" Impossible to build Portfolio Chart", err);
            }

		}
		#endregion

        #region Set series - static
        /// <summary>
		...
		private  void SetSeries(ChartArea chartArea, Dundas.Charting.WebControl.Series series, string[] xValues, double[] yValues, Color[] pieColors, string chartAreaName, bool typeFlash, string pieLineColor) {
        ...
        /// <param name="numberofChart">Number of chart</param>
        private void InitializeComponent(ChartArea chartArea, bool typeFlash, WebSession webSession, string defaultBorderLineColor, string borderLineColor, string titleColor, int indexTitle, int numberofChart, ref float posY)
        {
			#region Variables
            int individualChartHt = 285;           
            int chartHt = individualChartHt * numberofChart;
            float percentvalue = (float)100/ (float)(chartHt);
            ...
            this.Height = new Unit(Convert.ToString(chartHt));
            ...
            this.Titles[indexTitle].Font = new Font("Arial", (float)10);
            ...
            chartArea.Position.Height =  individualChartHt* percentvalue;
				
            this.Titles[indexTitle].Position.Y = (posY == 0) ? 3 : posY;
            chartArea.Position.Y = (posY == 0) ? 1 : posY;
            posY = posY + (individualChartHt * percentvalue);
