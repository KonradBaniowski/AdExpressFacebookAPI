	...
	public class ProofResultWebControl : System.Web.UI.WebControls.WebControl
	{
		...
		protected override void Render(HtmlTextWriter output)
		{
			...
			if (_dtResult!=null && _dtResult.Rows.Count > 0)
			{
			    ...
			    bool hasCopyRight = HasPressCopyright(Convert.ToInt64(_idMedia));
                ...
                if(hasCopyRight)
                {
                    foreach(DataRow row in _dtResult.Rows){
                        ...
                        if(File.Exists(pathCouv2)){
                            ...
                            if (row["datePublication"] != DBNull.Value) output.Write("<br><font class=txtViolet11>" + Dates.DateToString((DateTime)row["datePublication"], _customerWebSession.SiteLanguage) + "</font>");
                            if (row["number_page_media"] != DBNull.Value) output.Write("<br><font class=txtViolet11>" + row["number_page_media"].ToString() + "&nbsp;" + Convertion.ToHtmlString(GestionWeb.GetWebWord(UnitsInformation.List[CstWeb.CustomerSessions.Unit.pages].WebTextId, _customerWebSession.SiteLanguage)) + "</font>");
                        }
                        else{
                ...
                else
                {
                    ...
                    output.Write("<div align=\"center\" class=\"txtBlanc11Bold\">" + GestionWeb.GetWebWord(3015, _customerWebSession.SiteLanguage) + "</div>");
