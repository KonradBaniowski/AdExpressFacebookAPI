    ...
    public abstract class InsertionsDAL : IInsertionsDAL
    {
                                                            ...
                                                            , Int64 id, Table table)
        {
            ...
            if (vehicle.Id == Vehicles.names.mailValo)
                classifIds = ReplaceMailValoByVmc(vehicle, level, id);
        ...
        protected string ReplaceMailValoByVmc(VehicleInformation vehicle, DetailLevelItemInformation level, long id)
        {
        ...
        public virtual DataSet GetActiveDates(VehicleInformation vehicle, int fromDate, int toDate, List<GenericColumnItemInformation> columns)
        {
        ...
        public virtual List<string> GetActiveDates(VehicleInformation vehicle, int fromDate, int toDate, int universId, string filters)
        {
            var sql = new StringBuilder(5000);
            DataSet ds = new DataSet();
            List<string> activeperiods = new List<string>();
            try
            {
           ...
           * */
                ...
                #region query on all period

                //    // Select
                sql.Append(" select distinct substr(wp.date_media_num,0,6) date_media_num");
                ...
                // Append filters
                AppendUniversFilters(sql, tData, int.Parse(_session.PeriodBeginningDate), int.Parse(_session.PeriodEndDate), vehicle, universId, filters);
                ...
                foreach (DataRow dr in ds.Tables[0].Rows.Cast<DataRow>()
                 .Where(dr => !activeperiods.Contains(Convert.ToString(dr[0]))))
                {
                    activeperiods.Add(Convert.ToString(dr[0]));
                }
                ...
                int yearBegin = int.Parse(_session.PeriodBeginningDate.Substring(0, 4));
                int yearEnd = int.Parse(_session.PeriodEndDate.Substring(0, 4));


                #region Qyery by year

                ////Qyery by year
                //for (int i = yearBegin; i <= yearEnd; i++)
                //{
                //    string beginningDate = (i == yearBegin) ? _session.PeriodBeginningDate : string.Format("{0}0101", i);
                //    string endDate = (i==yearEnd) ? _session.PeriodEndDate : string.Format("{0}1231",i);


                //    if (i != yearBegin) sql.Append(" union  all   ");

                //    // Select
                //    sql.Append(" select distinct ");

                //    sql.Append(" to_char( to_date(wp.date_media_num ,'YYYYMMDD'),'YYYYMM')   date_media_num");
                ...
                //    // Append filters
                //    AppendUniversFilters(sql, tData, int.Parse(beginningDate), int.Parse(endDate), vehicle, universId, filters);

                //    //sql.Append("  group by to_char( to_date(wp.date_media_num ,'YYYYMMDD'),'YYYYMM')");

                //}

                #endregion

                #region Query by month

                //DateTime beginDate = new DateTime(int.Parse(_session.PeriodBeginningDate.Substring(0, 4)),
                //                                  int.Parse(_session.PeriodBeginningDate.Substring(4, 2)),
                //                                  int.Parse(_session.PeriodBeginningDate.Substring(6, 2)));

                //DateTime endDate = new DateTime(int.Parse(_session.PeriodEndDate.Substring(0, 4)),
                //                                int.Parse(_session.PeriodEndDate.Substring(4, 2)),
                //                                int.Parse(_session.PeriodEndDate.Substring(6, 2)));

                //File.AppendAllText(@"F:\TempsExection.txt",
                //                             string.Format(
                //                                 " Date début  ; date fin;  heure ; minute ; secondes ;  millisecondes \n"));
                //while (int.Parse(beginDate.ToString("yyyymm")) <= int.Parse(endDate.ToString("yyyymm")))
                //{
                //    string bDate = (string.Format("{0:yyyyMM}", beginDate) ==
                //                    _session.PeriodBeginningDate.Substring(0, 6))
                //                       ? _session.PeriodBeginningDate
                //                       : string.Format("{0}{1}01", beginDate.ToString("yyyy")
                //                                       , string.Format("{0:MM}", beginDate)
                //                             );
                //    string eDate = (string.Format("{0:yyyyMM}", beginDate) == _session.PeriodEndDate.Substring(0, 6))
                //                       ? _session.PeriodEndDate
                //                       : string.Format("{0}{1}31", beginDate.ToString("yyyy")
                //                                       , string.Format("{0:MM}", beginDate)
                //                             );
                ...
                //    //// Append filters
                //    //AppendUniversFilters2(sql, tData, int.Parse(bDate), int.Parse(eDate), vehicle, universId, filters);

                //    //sql.AppendFormat(" and rownum < 2 ");


                //    // Select
                //    sql.Append(" select count(*) date_media_num");
                ...
                //        File.AppendAllText(@"F:\TempsExection.txt",
                //                                     string.Format(
                //                                         " {4:yyyyMMdd} ;  {5:yyyyMMdd} ; {0} ; {1} ; {2}  ; {3} \n"
                //                                         , _duration.Hours, _duration.Minutes, _duration.Seconds,
                //                                         _duration.Milliseconds, bDate, eDate));
                //    else File.AppendAllText(@"F:\TempsExection.txt", string.Format("zero \n"));
                ...
                //    foreach (DataRow dr in ds.Tables[0].Rows.Cast<DataRow>()
                //                                       .Where(dr => Convert.ToInt64(dr[0]) > 0))
                //    {
                //        activeperiods.Add(eDate.Substring(0, 6));
                //    }
            ...
            {
                throw new InsertionsDALException(string.Format("Unable to build request to get creative active dates. {0}", sql.ToString()), err);
            }
        ...
        protected virtual void AppendUniversFilters2(StringBuilder sql, Table tData,
            int fromDate, int toDate, VehicleInformation vehicle, int universId, string filters)
        {
            ...
            if (_session.PeriodType == CstWeb.CustomerSessions.Period.Type.dateToDate)
            {
                ...
                {
                    sql.AppendFormat(" and {0}.date_creation >= to_date('{1} 00:00:00','yyyymmdd HH24:MI:SS') "
                        , tData.Prefix, fromDate);
                }
                ...
                if (_module.Id == CstWeb.Module.Name.NEW_CREATIVES)
                {
                    sql.AppendFormat(" and {0}.date_creation <= to_date('{1} 23:59:59','yyyymmdd HH24:MI:SS') "
                        , tData.Prefix, toDate);
                }
            ...
            if (_module.Id == CstWeb.Module.Name.ANALYSE_PLAN_MEDIA)
            {
                ...
                if (list.Length > 0 && vehicle.Id == Vehicles.names.internet) list = list.Replace(vehicle.DatabaseId.ToString(),
            ...
            if (_module.ModuleType == CstWeb.Module.Type.tvSponsorship)
            {
                sql.Append(SQLGenerator.getAdExpressUniverseCondition
                    (CstWeb.AdExpressUniverse.TV_SPONSORINGSHIP_MEDIA_LIST_ID,
                    tData.Prefix, tData.Prefix, tData.Prefix, true));
            }
            else
            {
                sql.Append(SQLGenerator.GetAdExpressProductUniverseCondition(CstWeb.
                    AdExpressUniverse.EXCLUDE_PRODUCT_LIST_ID, tData.Prefix, true, false));
            }
            /* This test is specific to the French version of the site
             * */
            string idCategoryThematic = TNS.AdExpress.Domain.Lists.GetIdList(CstWeb.GroupList
                .ID.category, CstWeb.GroupList.Type.thematicTv);
            if (!string.IsNullOrEmpty(idCategoryThematic))
            {
                sql.AppendFormat("  and  {0}.id_category not in ( {1}) ", tData.Prefix, idCategoryThematic);
            }
            ...
            if (vehicle.Id == Vehicles.names.internet)
                cVehicleInfo = VehiclesInformation.Get(Vehicles.names.adnettrack);
            else
                cVehicleInfo = vehicle;
            formatIdList = _session.GetValidFormatSelectedList(new List<VehicleInformation>(new[] { cVehicleInfo }));
            if (formatIdList.Count > 0)
                sql.AppendFormat(" and {0}.ID_{1} in ({2}) "
                    , tData.Prefix, WebApplicationParameters.DataBaseDescription.
                                                                                                                                                                                                                                                                                                                                                      GetTable(WebApplicationParameters.VehiclesFormatInformation.VehicleFormatInformationList[cVehicleInfo.DatabaseId].FormatTableName).Label, string.Join(",", formatIdList.ConvertAll(p => p.ToString()).ToArray()));
            #endregion
            ...
            if (slogans.Length > 0)
            {
                sql.AppendFormat(" and {0}.id_slogan in({1}) ", WebApplicationParameters.
                    DataBaseDescription.DefaultResultTablePrefix, slogans);
            }
            #endregion
            ...
            {
                sql.Append(SQLGenerator.
                    GetJointForInsertDetail(_session, WebApplicationParameters.
                    DataBaseDescription.DefaultResultTablePrefix));
            }

            switch (vehicle.Id)
            {
                case Vehicles.names.internet:
                    sql.AppendFormat(" and {1}.id_vehicle={0} ", VehiclesInformation.Get(Vehicles.
                        names.adnettrack).DatabaseId, tData.Prefix);
                    break;
                default:
