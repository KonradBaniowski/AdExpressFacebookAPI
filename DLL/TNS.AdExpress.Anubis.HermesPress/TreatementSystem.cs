...
namespace TNS.AdExpress.Anubis.HermesPress{
	...
	public class TreatementSystem : TNS.AdExpress.Anubis.BusinessFacade.ReportingPluginBase,TNS.AdExpress.Anubis.BusinessFacade.IPlugin{
		...
		/// <summary>
		/// Source de données pour la base pitagor (interrogation de la vue)
		/// </summary>
		...
		/// <summary>
		/// Nombre de support traité dont le résultat est 0 dans DATA_PRESS
		/// </summary>
		private int _nbMediaTreatedKO=0;
		/// <summary>
		/// Liste des supports traités dont le résultat est 0 dans DATA_PRESS
		/// </summary>
		...
		/// <summary>
		/// Traitement des vérifications des mises à jour des transferts de la presse sur la base AdExpress
		/// </summary>
		private void ComputeTreatement(){
			...
			try{
				...
				#region Vérification du transfert des données de chaque support de la Presse
					...
					try{
						...
						foreach(DataRow currentRow in dsParutionList.Tables[0].Rows){
							...
							try {
                                ...
                                #region Calcul du nombre de lignes de DATA_PRESS (Modifiée le 26/05/2008)
                                ...
                                // Compteur du nombre de parution à 0
                                if(nbLine == 0) {
                                    _nbMediaTreatedKO++;
								...
								error = true;
								OnError(_navSessionId,"Erreur dans la vérification de la mise à jour du Support ("+currentRow["id_media"].ToString()+") / Parution ("+currentRow["date_media_num"].ToString()+") > Rule ("+_ruleParameters.PluginId+") - Date : "+_ruleParameters.Day.ToString(), ex);
		...
		private string GetHtml(){
			...
			t.Append("</tr>");
			// Nombre total de parution vérifiée
			t.Append("<tr style=\"font-family: Arial, Helvetica, sans-serif;font-size: 13px;color: #4b3e5a;\" nowrap>");
			t.Append("<td>&nbsp;&raquo;&nbsp;Nombre total de parution vérifiée</td>");
			t.Append("<td><strong>"+_nbMediaTreated.ToString()+"</strong></td>");
			t.Append("</tr>");
			// Nombre de parution non transférée
			t.Append("<tr style=\"font-family: Arial, Helvetica, sans-serif;font-size: 13px;color: #4b3e5a;\" nowrap>");
			t.Append("<td>&nbsp;&raquo;&nbsp;Nombre de parution non transférée</td>");
