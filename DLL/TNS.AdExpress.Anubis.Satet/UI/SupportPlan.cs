                        ...
                        if (row.ItemArray[i] != DBNull.Value) {
                            if (row.Table.Columns[i].ColumnName.IndexOf("budget") > -1) {
                                ...
                                if (row.ItemArray[i].ToString().Equals("0"))
                                    ...
                                    SatetFunctions.WorkSheet.PutCellValue(cells, Convert.ToDouble(WebFunctions.Units.ConvertUnitValueAndPdmToString(Math.Round(Decimal.Parse(row.ItemArray[i].ToString()), 2).ToString(), WebConstantes.CustomerSessions.Unit.grp, false, WebApplicationParameters.AllowedLanguages[webSession.SiteLanguage].CultureInfo), WebApplicationParameters.AllowedLanguages[webSession.SiteLanguage].CultureInfo), cellRow - 1, i - 2, 1);
                                ...
                                SatetFunctions.WorkSheet.PutCellValue(cells, Math.Round(Decimal.Parse(PDM, WebApplicationParameters.AllowedLanguages[webSession.SiteLanguage].CultureInfo), 2), cellRow - 1, i - 2, 1);
