...
namespace TNS.AdExpress.Anubis.Mnevis
{
	...
	public class TreatementSystem:IPlugin {
		...
		public void Treatement(string configurationFilePath,IDataSource dataSource,Int64 navSessionId) {
            ...
            try {
                _theme = new Theme(new XmlReaderDataSource(AppDomain.CurrentDomain.BaseDirectory + _mnevisConfig.ThemePath + @"\App_Themes\" + WebApplicationParameters.Themes[((WebSession)_dataAccess.LoadData(_navSessionId)).SiteLanguage].Name + @"\" + "Styles.xml"));
            }
            catch (System.Exception err) {
