...
using TNS.AdExpress.Anubis.Selket.BusinessFacade;
using System.Data;
...
namespace TNS.AdExpress.Anubis.Selket
{
    ...
    : IPlugin
    {
        ...
        /// </summary>
        private SelketConfig _selketConfig;
        /// <summary>
        /// Theme
        /// </summary>
        ...
        {
            return ("Selket (VP) PDF Generator");
        ...
        public void Treatement(string configurationFilePath, IDataSource dataSource, Int64 navSessionId)
        {
             try {
                ...
                {
                    _dataAccess = null;
                    throw new SelketPdfException("Impossible to Create Instance Of Layer IStaticNavSessionDAL ", e);
                }
                #endregion
                ...
                {
                    throw new SelketPdfException("Impossible de lancer le traitement d'un job", new ArgumentNullException("Le nom du fichier de configuration est null."));
                }
                if (configurationFilePath.Length == 0)
                {
                    throw new SelketPdfException("Impossible de lancer le traitement d'un job", new ArgumentException("Le nom du fichier de configuration est vide."));
                }
                #endregion
                ...
                try
                {
                    _selketConfig = new SelketConfig(new XmlReaderDataSource(configurationFilePath));
                }
                catch (System.Exception err)
                {
                    throw new SelketPdfException("Impossible de lancer le traitement d'un job <== impossible de charger le fichier de configuration", err);
                }
                #endregion
                ...
                {
                    throw new SelketPdfException("Error for load session", err);
                }
                #endregion
                ...
                try
                {
                    ...
                    if (File.Exists(_selketConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml"))
                    {
                        pathFileTheme = _selketConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml";
                    }
                    else if (File.Exists(AppDomain.CurrentDomain.BaseDirectory + _selketConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml"))
                    {
                        pathFileTheme = AppDomain.CurrentDomain.BaseDirectory + _selketConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml";
                    }
                    else
                    {
                        pathFileTheme = _selketConfig.ThemePath + @"\" + WebApplicationParameters.Themes[_webSession.SiteLanguage].Name + @"\" + "Styles.xml";
                ...
                {
                    throw new SelketPdfException(string.Format("File of theme not found ! (in Plugin Hotep in TreatmentSystem class) - Path : '{0}'", pathFileTheme), err);
                }
                #endregion
             ...
             _myThread.Name = "Selket ( VP) PDF Generator";
             _myThread.Start();
        }
        ...
        private void ComputeTreatement()
        {
            ...
            try
            {
                ...
                pdf = new SelketPdfSystem(_dataSource, _selketConfig, rqDetails, _webSession, _theme);
                ...
                PluginInformation pluginInformation = PluginConfiguration.GetPluginInformation(PluginType.Selket);
