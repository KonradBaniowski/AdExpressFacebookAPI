    ...
    public class SelketPdfSystem : Pdf
    {
        ...
        protected virtual void VpFileResult()
        {
            ...
            if (_webSession.IdPromotion > -1)
            {
                ...
                var vpDAL = (IVeillePromoDAL)AppDomain.CurrentDomain.CreateInstanceFromAndUnwrap(string.Format("{0}Bin\\{1}",
                    AppDomain.CurrentDomain.BaseDirectory, module.CountryDataAccessLayer.AssemblyName), module.CountryDataAccessLayer.Class,
                ...
                if (ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        html.AppendFormat(" <li ><span><b>{0}:</b></span><span class=\"prfSp\"> {1}</span></li>"
                            , GestionWeb.GetWebWord(2876, _webSession.SiteLanguage), dr["BRAND"].ToString());
                        ...
                        html.AppendFormat(" <li ><span><b>{0}:</b></span><span class=\"prfSp\"> {1} {2} {3} {4}</span></li>",
                            GestionWeb.GetWebWord(895, _webSession.SiteLanguage), GestionWeb.GetWebWord(896, _webSession.SiteLanguage),
                            dateBegin, GestionWeb.GetWebWord(897, _webSession.SiteLanguage), dateEnd);

                        //Media type
                        if (dr["VEHICLE"] != DBNull.Value)
                        {
                            html.AppendFormat(" <li ><span><b>{0}:</b></span><span class=\"prfSp\"> {1}</span></li>"
                              , GestionWeb.GetWebWord(363, _webSession.SiteLanguage), dr["VEHICLE"].ToString());
                        }

                        //Product classification
                        html.AppendFormat("  <li ><span><b>{0}:</b></span><span class=\"prfSp\"> {1} /{2} / {3}</span></li>",
                            GestionWeb.GetWebWord(2884, _webSession.SiteLanguage), dr["SEGMENT"].ToString(), dr["CATEGORY"].ToString(), dr["PRODUCT"].ToString());
                        ...
                        if (dr["PROMOTION_CONTENT"] != System.DBNull.Value && dr["PROMOTION_CONTENT"].ToString().Length > 0)
                        {
                            ...
                            html.AppendFormat("  <li><span><b>{0}:</b></span><span class=\"prfSp\"> {1}</span></li>",
                                GestionWeb.GetWebWord(2885, _webSession.SiteLanguage), promoContent);
                        }

                        //Exclu Web
                        html.AppendFormat(" <li><span><b>{0}:</b></span><span class=\"prfSp\"\"> {1}</span></li>"
                            , GestionWeb.GetWebWord(2997, _webSession.SiteLanguage),
                            (Convert.ToInt64(dr["EXCLU_WEB"].ToString()) == 1) ? GestionWeb.GetWebWord(2998, _webSession.SiteLanguage)
                            : GestionWeb.GetWebWord(2999, _webSession.SiteLanguage));

                        //Brands
                        if (dr["PROMOTION_BRAND"] != DBNull.Value && dr["PROMOTION_BRAND"].ToString().Length > 0)
                        {
                        ...
                        if (vInfo.Id != Vehicles.names.radio && dr["PROMOTION_VISUAL"] != DBNull.Value
                            && dr["PROMOTION_VISUAL"].ToString().Length > 0)
                        {

                            string[] promoVisuals = null;
                            if (vInfo.Id == Vehicles.names.tv && dr["TV_BOARD"] != DBNull.Value)
                            {
                                promoVisuals = dr["TV_BOARD"].ToString().Split(',');
                            }
                            else if (IsImages(vInfo, dr))
                            {
                                promoVisuals = dr["PROMOTION_VISUAL"].ToString().Split(',');
                            }

                            AddpromoVisuals(promoVisuals, vInfo);
                        }
        ...
        protected virtual void AddpromoVisuals(string[] imgList, VehicleInformation vehicleInformation)
        {
            if (imgList != null && imgList.Length > 0)
            {
                ...
                {
                    string visualPromo = GetCreativePath(vehicleInformation, imgList[i]);
        ...
        private string GetCreativePath(VehicleInformation vehicleInformation, string fileName)
        {
            switch (vehicleInformation.Id)
            {
                case Vehicles.names.webPromotion:
                    return WebConstantes.CreationServerPathes.LOCAL_PATH_VP + fileName;
                case Vehicles.names.internet:
                    return string.Format(@"{0}{1}\{2}\{3}", WebConstantes.CreationServerPathes.LOCAL_PATH_BANNERS_PSA
                         , fileName.Substring(0, 1), fileName.Substring(1, 3),
                                                              fileName);
                case Vehicles.names.press:
                case Vehicles.names.outdoor:
                case Vehicles.names.tv:
                    return string.Format(@"{0}{1}\{2}\{3}", WebConstantes.CreationServerPathes.LOCAL_PATH_IMAGES_PSA_MR
                           , fileName.Substring(0, 1), fileName.Substring(1, 3),
                                                                fileName);
                default:
                    throw new Exception(" Cannot identify Media type Identifier");
            }
        ...
        private bool IsImages(VehicleInformation vInfo, DataRow dr)
        {
            string fileExtension = dr["PROMOTION_VISUAL"] != DBNull.Value
                                       ? Path.GetExtension(dr["PROMOTION_VISUAL"].ToString().Trim())
                                       : string.Empty;
            return !string.IsNullOrEmpty(fileExtension) && !(vInfo.Id == Vehicles.names.tv ||
                vInfo.Id == Vehicles.names.radio ||
