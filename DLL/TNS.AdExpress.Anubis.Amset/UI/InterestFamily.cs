			...
			try{
				...
				if(resultTable.LinesNumber>0){
					...
					if(webSession.Unit==WebConstantes.CustomerSessions.Unit.grp){
						...
						AmsetFunctions.WorkSheet.PageSettings(sheet, GestionWeb.GetWebWord(1777, webSession.SiteLanguage), (int)resultTable.LinesNumber + ADD_NB_LINES + 1, 42, 8, "", "1", webSession.SiteLanguage);
					}
					else{
						cells.SetColumnWidth((byte)FIRST_SHEET_COLUMN,30);
						cells.SetColumnWidth((byte)SECOND_SHEET_COLUMN,20);
						cells.SetColumnWidth((byte)THIRD_SHEET_COLUMN,18);
						AmsetFunctions.WorkSheet.PageSettings(sheet, GestionWeb.GetWebWord(1777, webSession.SiteLanguage), (int)resultTable.LinesNumber + ADD_NB_LINES, 42, 10, "", "1", webSession.SiteLanguage);
					}
				}
			}
			catch(Exception e){
