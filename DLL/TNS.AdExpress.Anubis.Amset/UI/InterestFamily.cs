...
namespace TNS.AdExpress.Anubis.Amset.UI{
	/// <summary>
	/// Description résumée de InterestFamily.
	/// </summary>
	public class InterestFamily{
        ...
        private static string _rowDefault = "InterestFamilyRowDefault";
        private static string _rowTotal = "InterestFamilyRowTotal";
        #endregion

		#region InterestFamily
		/// <summary>
		/// InterestFamily
		/// </summary>
        internal static void SetExcelSheet(Workbook excel, WebSession webSession, IDataSource dataSource, TNS.FrameWork.WebTheme.Style style) {
			...
			try{
				...
				ResultTable resultTable = APPMRules.AnalyseFamilyInterestPlanRules.GetFamilyInterestResultTable(webSession,webSession.Source,idWave,int.Parse(webSession.PeriodBeginningDate),int.Parse(webSession.PeriodEndDate),idBaseTarget,idAdditionalTarget);					
				...
				if(resultTable.LinesNumber>0){
					...
					for(int i=0; i<resultTable.LinesNumber;i++){
						...
						if(webSession.Unit==WebConstantes.CustomerSessions.Unit.grp)
                            ...
                            AmsetFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(tagName), null, cellRow, FIRST_SHEET_COLUMN, THIRD_SHEET_COLUMN, false);
                        ...
                        AmsetFunctions.WorkSheet.PutCellValue(excel, sheet, cells, style.GetTag(tagName), valeur, cellRow, SECOND_SHEET_COLUMN, SECOND_SHEET_COLUMN);
					...
					if(webSession.Unit==WebConstantes.CustomerSessions.Unit.grp){
						cells.SetColumnWidth((byte)FIRST_SHEET_COLUMN,30);
                        ...
                        AmsetFunctions.WorkSheet.PageSettings(sheet, GestionWeb.GetWebWord(1777, webSession.SiteLanguage), (int)resultTable.LinesNumber + ADD_NB_LINES + 1, 42, 8, "", "1", webSession.SiteLanguage, style);
					}
					else{
						cells.SetColumnWidth((byte)FIRST_SHEET_COLUMN,30);
						cells.SetColumnWidth((byte)SECOND_SHEET_COLUMN,20);
						cells.SetColumnWidth((byte)THIRD_SHEET_COLUMN,18);
                        AmsetFunctions.WorkSheet.PageSettings(sheet, GestionWeb.GetWebWord(1777, webSession.SiteLanguage), (int)resultTable.LinesNumber + ADD_NB_LINES, 42, 10, "", "1", webSession.SiteLanguage, style);
					}
				}
			}
			catch(Exception e){
				throw(new AmsetExceptions.AmsetExcelSystemException("Unable to build the interest family page.",e));
			}
		}
		#endregion
