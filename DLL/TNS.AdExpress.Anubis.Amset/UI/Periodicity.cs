...
namespace TNS.AdExpress.Anubis.Amset.UI{
	/// <summary>
	/// Description résumée de Periodicity.
	/// </summary>
	public class Periodicity{
        ...
        private static string _rowDefault = "PeriodicityRowDefault";
        private static string _rowTotal = "PeriodicityRowTotal";
        #endregion
        ...
        internal static void SetExcelSheet(Workbook excel, WebSession webSession, IDataSource dataSource, TNS.FrameWork.WebTheme.Style style) {
			...
			try{
				...
				ResultTable resultTable = APPMRules.PeriodicityPlanRules.GetPeriodicityResultTable(webSession,webSession.Source,idWave,int.Parse(webSession.PeriodBeginningDate),int.Parse(webSession.PeriodEndDate),idBaseTarget,idAdditionalTarget);					
				...
				if(resultTable.LinesNumber>0){
					...
					if(webSession.Unit==WebConstantes.CustomerSessions.Unit.grp){
						...
						AmsetFunctions.WorkSheet.PageSettings(sheet, GestionWeb.GetWebWord(1293, webSession.SiteLanguage), (int)resultTable.LinesNumber, 42, 8, "", "1", webSession.SiteLanguage,style);
					}
					else{
						cells.SetColumnWidth((byte)FIRST_SHEET_COLUMN,30);
						cells.SetColumnWidth((byte)SECOND_SHEET_COLUMN,20);
						cells.SetColumnWidth((byte)THIRD_SHEET_COLUMN,18);
                        AmsetFunctions.WorkSheet.PageSettings(sheet, GestionWeb.GetWebWord(1293, webSession.SiteLanguage), (int)resultTable.LinesNumber, 42, 10, "", "1", webSession.SiteLanguage, style);
					}
				}
			}
			catch(Exception e){
				throw(new AmsetExceptions.AmsetExcelSystemException("Unable to build the periodicity page.",e));
			}
		}
		#endregion
