			...
			try{
				...
				if(resultTable.LinesNumber>0){
					...
					if(webSession.Unit==WebConstantes.CustomerSessions.Unit.grp){
						...
						AmsetFunctions.WorkSheet.PageSettings(sheet,GestionWeb.GetWebWord(1293,webSession.SiteLanguage),(int)resultTable.LinesNumber,42,8,"","1");
					}
					else{
						cells.SetColumnWidth((byte)FIRST_SHEET_COLUMN,30);
						cells.SetColumnWidth((byte)SECOND_SHEET_COLUMN,20);
						cells.SetColumnWidth((byte)THIRD_SHEET_COLUMN,18);
						AmsetFunctions.WorkSheet.PageSettings(sheet,GestionWeb.GetWebWord(1293,webSession.SiteLanguage),(int)resultTable.LinesNumber,42,10,"","1");
					}
				}
			}
			catch(Exception e){
