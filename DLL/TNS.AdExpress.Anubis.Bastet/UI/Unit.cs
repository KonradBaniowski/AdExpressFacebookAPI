		...
		internal static Excel TopUsed(Excel excel,BastetCommon.Parameters parameters) {
			try{
				...
				if(dt!=null && dt.Rows.Count>0){
					...
					sheet.Name="Top unités utilisées"; // A mettre dans web word
					...
					//Top unités utilisés
					cells["B"+cellRow].PutValue(" Top unités utilisées ");// A mettre dans web word
					...
					for(int i=0; i<dt.Rows.Count;i++){	
						//				    unitWord = GestionWeb.GetWebWord((int)SessionCst.UnitsTraductionCodes[(SessionCst.Unit)int.Parse(dt.Rows[i]["id_unit"].ToString())],Language.FRENCH.GetHashCode());					
						//					unitWord = DataAccess.Unit.Label(parameters,dt.Rows[i]["id_unit"].ToString());
