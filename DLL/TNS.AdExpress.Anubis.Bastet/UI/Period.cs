...
namespace TNS.AdExpress.Anubis.Bastet.UI
{
	...
	public class Period
	{
		...
		/// <summary>
		/// Top des périodes les plus utilisés dans AdExpress 
		/// </summary>		
		...
		internal static Workbook TopUsed(Workbook excel,BastetCommon.Parameters parameters,int language) {
			try{
				...
				if(dt!=null && dt.Rows.Count>0){
					...
					sheet.Name = GestionWeb.GetWebWord(2523, language); // A mettre dans web word
					...
					cells["B"+cellRow].PutValue(" "+GestionWeb.GetWebWord(2523, language)+" ");
					...
					for(int i=0; i<dt.Rows.Count;i++){
                        cells["B" + cellRow].PutValue(GestionWeb.GetWebWord(PeriodsInformation.Get(Int64.Parse(dt.Rows[i]["ID_PERIODE"].ToString())).WebTextId,language));
				...
				throw (new  BastetExceptions.PeriodUIException(" TopUsed : Impossible d'obtenir la liste des périodes les plus utilisés ", err));
			}
			return excel;
		}
