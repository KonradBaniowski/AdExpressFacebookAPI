	...
	public class Period
	{
		...
		public static DataTable TopUsed(BastetCommon.Parameters parameters) {
			try{
				...
				//Where
                sql.Append(" where " + topPeriod.Prefix + ".date_connection  between " + parameters.PeriodBeginningDate.ToString("yyyyMMdd") + " and " + parameters.PeriodEndDate.ToString("yyyyMMdd"));
