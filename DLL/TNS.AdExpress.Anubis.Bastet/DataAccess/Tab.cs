	...
	public class Tab
	{
		...
		public static DataTable TopUsed(BastetCommon.Parameters parameters) {
			try{
				...
				//Where
                sql.Append(" where " + topOption.Prefix + ".date_connection  between " + parameters.PeriodBeginningDate.ToString("yyyyMMdd") + " and " + parameters.PeriodEndDate.ToString("yyyyMMdd"));
