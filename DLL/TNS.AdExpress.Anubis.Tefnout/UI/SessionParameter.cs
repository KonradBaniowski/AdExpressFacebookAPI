...
namespace TNS.AdExpress.Anubis.Tefnout.UI
{
	...
	public class SessionParameter
	{
        ...
        internal static void SetExcelSheet(Workbook excel, WebSession webSession, IDataSource dataSource, TNS.FrameWork.WebTheme.Style style) {
            ...
            TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagWaveTitle), null, cellRow - 1, 1, 3, false);
			cellRow++;
            TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagWaveValue), ((LevelInformation)webSession.SelectionUniversAEPMWave.Nodes[0].Tag).Text, cellRow - 1, 1, 1, false);
			cells[cellRow-1,1].Style.IndentLevel=1;
			cellRow+=2;
			#endregion
            ...
            TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagTargetTitle), null, cellRow - 1, 1, 3, false);
			cellRow++;
			...
			foreach(DataRow r in ds.Tables[0].Rows){
                TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagTargetValue), r["target"].ToString(), cellRow - 1, 1, 1, false);
				cells[cellRow-1,1].Style.IndentLevel=1;
				cellRow++;
			}
			ds.Dispose();
			...
			cellRow++;
			//reference
            TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagProductValue), GestionWeb.GetWebWord(1677, webSession.SiteLanguage) + " :", cellRow - 1, 1, 1, false);
			cellRow+=2;
			...
			if (webSession.PrincipalProductUniverses.Count>1){
                TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagProductCompetitor), GestionWeb.GetWebWord(1678, webSession.SiteLanguage) + " :", cellRow - 1, 1, 1, false);
				cellRow+=2;
			}
			else{
                TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagProductCompetitor), GestionWeb.GetWebWord(1668, webSession.SiteLanguage) + " :", cellRow - 1, 1, 1, false);
				cellRow+=2;
			...
			//else{
				...
				for(int i =0; i < ds.Tables[0].Rows.Count; i++){
                    ...
                    TefnoutFunctions.WorkSheet.CellsStyle(excel, cells, style.GetTag(_tagProductPrincipal), null, cellRow - 1, 1, 3, false);
					cellRow++;
				}
				ds.Dispose();
				ds = null;
        ...
        public static void ToExcel(TreeNode root, Workbook excel, WebSession webSession, Worksheet sheet, ref int cellRow, Cells cells, TNS.FrameWork.WebTheme.Style style) {
			...
			int nbTD=1;
            TNS.AdExpress.Anubis.Tefnout.UI.SessionParameter.ToExcel(root, 0, maxLevel - 1, ref nbTD, excel, webSession, sheet, ref cellRow, cells, style);
		}
