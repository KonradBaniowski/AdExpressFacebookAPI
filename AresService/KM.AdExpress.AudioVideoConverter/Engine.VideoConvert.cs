                ...
                using (var db = dal.AudioVideoConverterDB.CreateDbManager())
                {
                    ...
                    creatives.ForEach(p =>
                    {
                        if (!string.IsNullOrEmpty(p.FileName) && p.IdMultimedia > 0)
                        {
                            ...
                            string cFullSourceFilename = string.Format("{0}.mp4", Path.Combine(p.FilePath, p.FileName));
                            bool isMp4FileExists = isFileSourceExist = File.Exists(cFullSourceFilename);


                            if (!isMp4FileExists)
                            {
                                cFullSourceFilename = string.Format("{0}.avi", Path.Combine(p.FilePath, p.FileName));
                                isFileSourceExist = File.Exists(cFullSourceFilename);
                            }


                            string cFullDestinationFileName = Path.Combine(TvDestinationPath, string.Format(@"{0}\", p.MediaYear.ToString()));

                            if (!isFileSourceExist)
                            {
                            ...
                            if (isFileSourceExist && !isExistingDestFile)
                            {
                                ...
                                {
                                    File.Copy(cFullSourceFilename, cHrFullDestinationFileName, true);
                                    File.Copy(cFullSourceFilename, cMrFullDestinationFileName, true);
                                    File.Copy(cFullSourceFilename, cBrFullDestinationFileName, true);
                                    AdvancedTrace.TraceInformation(string.Format("La Version {0} possede deja un fichier source  {1}  au format MP4...", p.IdMultimedia.ToString(), p.FileName), ModuleName);
                                    reportGenerator.NbExistingMp4SourceFiles++;
                                }
