...
namespace AdExpress.Private.Selection.Russia
{
    ...
    public partial class ReferenceCompetitorPersonalisation : TNS.AdExpress.Web.UI.SelectionWebPage
    {
        ...
        /// Constructeur
        /// </summary>
        public ReferenceCompetitorPersonalisation()
            : base()
        {
        }
        #endregion
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            try
            {
                ...
                // Bouton ok
                if (Request.Form.Get("__EVENTTARGET") == "OkImageButtonRollOverWebControl")
                {
                    eventButton = constEvent.eventSelection.OK_EVENT;
                }
                //Button initalize search
                ...
                if (validateButton.CommandName == "next")
                {
                ...
                recapAdvertiserSelectionWebControl.KeyWord = keyWordTextBox.Text;
                recapAdvertiserSelectionWebControl.EventButton = eventButton;
        ...
        protected void validateButton_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (button.CommandName == "next")
                {
                    ...
                    {
                        advertisers = GetAdvertiserSelection();
                        this.recapAdvertiserSelectionWebControl.CheckedAdvertisers = advertisers;
        ...
        protected void OkImageButtonRollOverWebControl_Click(object sender, System.EventArgs e)
        {
            try
            {
                List<long> advertisers = null;
                NomenclatureElementsGroup nomenclatureElementsGroup = null;

                advertisers = GetAdvertiserSelection();
                this.recapAdvertiserSelectionWebControl.CheckedAdvertisers = advertisers;
                if (validateButton.CommandName == "validate")
                {
                    if (_webSession.SecondaryProductUniverses != null && _webSession.SecondaryProductUniverses.Count > 0 && _webSession.SecondaryProductUniverses[0].Count() > 0)
                        nomenclatureElementsGroup = _webSession.SecondaryProductUniverses[0].GetGroup(0);
                    this.recapAdvertiserSelectionWebControl.ExceptionsList = (nomenclatureElementsGroup != null && nomenclatureElementsGroup.Count() > 0) ? nomenclatureElementsGroup.GetAsString(TNSClassificationLevels.ADVERTISER) : "";
                    NextToValidate();
                }
