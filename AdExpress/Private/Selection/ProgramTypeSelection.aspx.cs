...
namespace AdExpress.Private.Selection 
{
	...
	/// </summary>
	public partial class ProgramTypeSelection : TNS.AdExpress.Web.UI.SelectionWebPage {
		...
		/// </summary>
		/// <summary>
		/// Texte : Genre d'émission
		/// </summary>
		/// <summary>
		/// Texte : Saississez votre mot-clé
		/// </summary>
		/// <summary>
		/// Texte box : Pour la saisie du genre d'émission (mot-clé)
		/// </summary>
		/// <summary>
		...
		/// </summary>
		/// <summary>
		/// Contrôle : liste des genres d'émissions
		/// </summary>
		/// <summary>
		/// contrôle Recall
		/// </summary>
		...
		/// Constructeur
		/// </summary>
		public ProgramTypeSelection():base() {
		}
		#endregion
		...
		protected void Page_Load(object sender, System.EventArgs e) {
			try {
				...
				if(Page.Request.Form.GetValues("__EVENTTARGET")!=null){
					...
					if(nomInput!=validateButton.ID && nomInput!=MenuWebControl2.ID && nomInput!=formSponsoringshipImageButtonRollOverWebControl.ID){
				...
				else if(Request.Form.Get("__EVENTTARGET")=="formSponsoringshipImageButtonRollOverWebControl") {
				...
				AdvertiserSelectionWebControl1.ProgramTypeText=textBoxProgramTypeChoice.Text;	
				if(eventButton!=4) {
		...
		protected override System.Collections.Specialized.NameValueCollection DeterminePostBackMode() {
			...
			LoadableUniversWebControl1.ListBranchType=TNS.AdExpress.Constantes.Classification.Branch.type.programType.GetHashCode().ToString();//+","+TNS.AdExpress.Constantes.Classification.Branch.type.program.GetHashCode().ToString();
		...
		protected override void OnPreRender(EventArgs e) {
			try {
				...
				if(IsPostBack) {
					if(AdvertiserSelectionWebControl1.DsListProgramType!=null) {
						if(AdvertiserSelectionWebControl1.DsListProgramType.Tables[0].Rows.Count==0) {
		...
		protected void validateButton_Click(object sender, System.EventArgs e) {
			try {
				_webSession.CurrentUniversProgramType=(System.Windows.Forms.TreeNode)_webSession.SelectionUniversProgramType.Clone();
				_webSession.Save();
				...
				if(AdvertiserSelectionWebControl1.NbElement==1) {
					...
					// Erreur : 1 élément
					Response.Write("<script language=javascript>");
					Response.Write("alert(\""+GestionWeb.GetWebWord(2060,_webSession.SiteLanguage)+"\");");
					Response.Write("history.go(-1);");
					Response.Write("</script>");
				}
			}
		...
		#region Boutton Forme de parrainage
		/// <summary>
		/// Bouton Forme de parrainage
		/// </summary>
		/// <param name="sender">Objet qui lance l'évènement</param>
		...
		protected void saveUniverseImageButtonRollOverWebControl_Click(object sender, System.EventArgs e) {
			...
			TNS.AdExpress.Constantes.Classification.Branch.type branchType=TNS.AdExpress.Constantes.Classification.Branch.type.programType;
			
			idUniverseClientDescription=WebConstantes.LoadableUnivers.PROGRAM_TYPE_PROGRAM;
			branchType=TNS.AdExpress.Constantes.Classification.Branch.type.programType;
		...
		protected bool loadUniversAdvertiser() {
			...
			if(idUniverse!=0) {
				...
				_webSession.SelectionUniversProgramType=treeNodeUniverse;
				_webSession.Save();
