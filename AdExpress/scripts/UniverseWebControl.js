...
function ob_t2_Custom_Add(parentId, currentLevelId, treeviewId, oSelect, hiddenFieldId, nodeCss, expanded, image, subTreeURL) {
    capacityExceptionMessage = "" + capacityExceptionMessage + "";
        ...
        if (oSelect.options[i].selected && currentLevelId != null && currentLevelId.length > 0 && document.getElementById(childId) == null) { //Debut Si  un élément sélectionné	  
            ...
            if (hiddenFieldIElement != null) {
                if (hiddenFieldIElement.value != null && hiddenFieldIElement.value.length > 0) {
                    if (!(hiddenFieldIElement.value.split(',').length < nbMaxItemByLevel)) {
...
/*Fonction personalisées D'ajout d'éléments*/
//Adex_
function ob_t2_Custom_Add(parentId, currentLevelId, treeviewId, oSelect, hiddenFieldId, nodeCss, expanded, image, subTreeURL, levelsIdToReadOnly, cannotAddLevelMessage) {
    ...
    for (i = 0; i < oSelect.options.length; i++) {//Accolade Boucle debut Ajout des éléments d'ans un niveau
        ...
        if (oSelect.options[i].selected && currentLevelId != null && currentLevelId.length > 0 && document.getElementById(childId) == null) { //Debut Si  un élément sélectionné	  
            if (!ob_hasChildren(pNode)) {
                if (currentLevelId != null && currentLevelId.length > 0 && levelsIdToReadOnly != null && levelsIdToReadOnly.length > 0) {
                    levelsIdToReadOnly_Array = levelsIdToReadOnly.split(',');

                    for (r = 0; r < levelsIdToReadOnly_Array.length; r++) {
                        if (levelsIdToReadOnly_Array[r] != null && typeof (levelsIdToReadOnly_Array[r]) != "undefined" && levelsIdToReadOnly_Array[r].length > 0 && (parseInt(levelsIdToReadOnly_Array[r]) == parseInt(currentLevelId))) {
                            alert(cannotAddLevelMessage);
                            return;
                        }
                    }
