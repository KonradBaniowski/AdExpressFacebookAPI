...
namespace TNS.AdExpress.Vehicle.DAL
{
    public class VehicleListDataAccess
    {
        #region CONSTANTS
        private const string DSLISTADVERTISER = "dsListAdvertiser";
        private const string DSLISTMEDIA = "dsListMedia";
        #endregion
        ...
        public VehicleListDataAccess(WebSession webSession)
        {
            ...
            try
            {
                ...
                if (webSession.CurrentModule == WebConstantes.Module.Name.INDICATEUR
        ...
        public static DataSet DetailInterestCenterListDataAccess(WebSession webSession)
        {
            ...
            if (VehiclesInformation.Contains(DBClassification.Vehicles.names.internet) && ((LevelInformation)webSession.SelectionUniversMedia.FirstNode.Tag).ID == VehiclesInformation.EnumToDatabaseId(DBClassification.Vehicles.names.internet))
            {
                ...
                string inClauseSQLCode = SQLGenerator.GetInClauseMagicMethod(DBConstantes.Tables.MEDIA_PREFIXE + ".id_media", activeMediaList);
            ...
            if (Modules.IsSponsorShipTVModule(webSession))
              sql.Append(SQLGenerator.getAdExpressUniverseCondition(WebConstantes.AdExpressUniverse.TV_SPONSORINGSHIP_MEDIA_LIST_ID, true));
        ...
        public static DataSet VehicleInterestCenterMediaListDataAccess(WebSession webSession, string listVehicle, string listInterestCenter, string listMedia)
        {
            ...
            #region Media rights
            sql.Append(SQLGenerator.getClassificationCustomerRecapMediaRight(webSession, vehicleTable.Prefix, categoryTable.Prefix, mediaTable.Prefix, true));
            #endregion
        ...
        public static DataSet InterestCenterList(WebSession webSession, string idInterestCenter, string idMedia)
        {
            ...
            //sélection média 
            if (CheckedText.CheckedProductText(idInterestCenter) || CheckedText.CheckedProductText(idMedia))
            {
                sql.Append("  and  ( ");
            ...
            #region droits média
            sql.Append(SQLGenerator.getClassificationCustomerRecapMediaRight(webSession, oView.Prefix, oView.Prefix, oView.Prefix, true));
            #endregion
        ...
        protected string GetRecapMediaConditions(WebSession webSession, Table vehicleTable, Table categoryTable, Table mediaTable)
        {
            ...
            sql += SQLGenerator.getAccessVehicleList(webSession, vehicleTable.Prefix, true);
            if (webSession.CurrentModule == WebConstantes.Module.Name.INDICATEUR
                || webSession.CurrentModule == WebConstantes.Module.Name.TABLEAU_DYNAMIQUE
        ...
        protected string GetMediaRights(WebSession webSession, Table vehicleTable, Table categoryTable, Table mediaTable)
        {
            ...
            if (webSession.CustomerLogin[CustomerRightConstante.type.vehicleAccess].Length > 0)
            {
                ...
                sql += " ((" + SQLGenerator.GetInClauseMagicMethod(vehicleTable.Prefix + ".id_vehicle", webSession.CustomerLogin[CustomerRightConstante.type.vehicleAccess], true) + " ";
                premier = false;
            }
            // Category
            if (webSession.CustomerLogin[CustomerRightConstante.type.categoryAccess].Length > 0)
            {
                ...
                sql += " " + SQLGenerator.GetInClauseMagicMethod(categoryTable.Prefix + ".id_category", webSession.CustomerLogin[CustomerRightConstante.type.categoryAccess], true) + " ";
                premier = false;
            }
            // Media
            if (webSession.CustomerLogin[CustomerRightConstante.type.mediaAccess].Length > 0)
            {
                ...
                sql += " " + SQLGenerator.GetInClauseMagicMethod(mediaTable.Prefix + ".id_media", webSession.CustomerLogin[CustomerRightConstante.type.mediaAccess], true) + " ";
                premier = false;
            }
            if (!premier) sql += " )";
            ...
            if (webSession.CustomerLogin[CustomerRightConstante.type.vehicleException].Length > 0)
            {
                ...
                sql += " " + SQLGenerator.GetInClauseMagicMethod(vehicleTable.Prefix + ".id_vehicle", webSession.CustomerLogin[CustomerRightConstante.type.vehicleException], false) + " ";
                premier = false;
            }
            // Category
            if (webSession.CustomerLogin[CustomerRightConstante.type.categoryException].Length > 0)
            {
                ...
                sql += " " + SQLGenerator.GetInClauseMagicMethod(categoryTable.Prefix + ".id_category", webSession.CustomerLogin[CustomerRightConstante.type.categoryException], false) + " ";
                premier = false;
            }
            // Media
            if (webSession.CustomerLogin[CustomerRightConstante.type.mediaException].Length > 0)
            {
                ...
                sql += " " + SQLGenerator.GetInClauseMagicMethod(mediaTable.Prefix + ".id_media", webSession.CustomerLogin[CustomerRightConstante.type.mediaException], false) + " ";
                premier = false;
            }
            if (!premier) sql += " )";
            #endregion
        ...
        protected string GetSqlQuery(WebSession webSession)
        {
            ...
            if (webSession.CurrentModule == WebConstantes.Module.Name.INDICATEUR)
            {
                List<Int64> lst = ((WebNavigation.Module)webSession.CustomerLogin.GetModule(webSession.CurrentModule)).ExcludedVehicles;
            ...
            if (webSession.CurrentModule == WebConstantes.Module.Name.NEW_CREATIVES)
            {
